...
#[cfg(test)]
use toniator::GuidedDefinitionCatalogEntry;
#[cfg(test)]
...
use toniator::pattern::{
    ArtboardSpace, PATTERN_REGISTRY, PatternId, PatternInspectorPanel, PatternOutputKind,
};
use toniator::pattern_definition::{
    ...
    ParameterApplicability, ParameterAuthoring, ParameterInvalidationScope, ParameterOwnership,
    ParameterSerializationBehavior, ParameterValidationBehavior, PatternInstanceValue,
    PatternParameterConstraints, PatternParameterDefinition, RecipeValueType,
    TwoDimensionalRelation,
};
use toniator::persistence::{
    DocumentOpenCandidate, MissingPatternDefinitionOpenCandidate, clear_recovery_if_matches,
    recovery_path,
};
#[cfg(test)]
use toniator::render_document_preview;
use toniator::{
    AlternateTileTransform, AuthoringSection, CancellationToken, CurveLayout, CurvePath,
    CurvePoint, DistributionLimits, Document, DocumentAppearance, DocumentEditor, ExportBackground,
    ExternalPatternDefinitionImport, ExternalPatternDefinitionImportChoice,
    ExternalPatternDefinitionImportPlan, GuidedDefinitionCatalog, GuidedDefinitionDraft,
    GuidedWidgetKind, Ink, MotifCoverage, OperationCancelled, OutputMode,
    PatternDefinitionLifecycleResolver, PatternDefinitionSource, PreviewSurface, RenderGate,
    RgbaColor, Settings, SharedRecipeEditorDraft, Treatment, ValueMode, WebCurveChannel,
    WebCurveSettings, WebShape, WebShapeSettings, commit_external_pattern_definition_import,
    export_svg, export_svg_cancellable, inspect_external_pattern_definition_import,
    load_bundled_pattern_definition_registry, native_user_pattern_library_dir,
    project_definition_for_library_save, render_canonical_pattern_output_cancellable,
    render_document_preview_cancellable, save_document_atomic, save_user_pattern_definition,
};
...
const MISSING_PATTERN_RECOVERY_CANCEL_RESPONSE: &str = "Cancel";
const MISSING_PATTERN_RECOVERY_LOCATE_RESPONSE: &str = "Locate Matching .tnpattern";
const MISSING_PATTERN_RECOVERY_REPLACEMENT_RESPONSE: &str = "Use Replacement (Discard Values)";
static UI_RESOURCES_REGISTERED: Once = Once::new();

fn missing_pattern_replacement_label(
    candidate: &toniator::MissingPatternDefinitionReplacementCandidate,
) -> String {
    format!(
        "{} — {} — {}",
        candidate.name, candidate.provenance, candidate.id
    )
}

fn missing_pattern_recovery_feedback_summary(detail: &str) -> &'static str {
    if detail.starts_with("Matching recovery") {
        "No match: choose a current .tnpattern with the requested ID."
    } else if detail.starts_with("Replacement recovery") {
        "Replacement failed. Choose another definition or Cancel."
    } else if detail.starts_with("Recovered candidate") {
        "Recovery could not load the artwork. Choose another matching definition."
    } else {
        "Recovery did not change the document. Review the recovery details."
    }
}
...
fn user_pattern_dir(data_home: Option<&Path>, home: Option<&Path>) -> PathBuf {
    toniator::user_pattern_library_dir(data_home, home)
}
...
fn pattern_curve_function_from_choice(value: &str) -> Result<PatternCurveFunction, String> {
    match value {
        "sine" => Ok(PatternCurveFunction::Sine),
        "square" => Ok(PatternCurveFunction::Square),
        "sawtooth" => Ok(PatternCurveFunction::Sawtooth),
        unsupported => Err(format!(
            "Curve Function `{unsupported}` is not supported by the current Pattern Editor; obsolete recipe values must be replaced explicitly"
        )),
    }
}
...
fn editor_number_parameter(
    ...
    help: &str,
    default: f64,
    bounds: EditorNumberBounds,
    scope: DefinitionParameterScope,
    authoring: ParameterAuthoring,
...
struct EditorCreatorContract {
    ...
    invalidation: ParameterInvalidationScope,
    two_dimensional: Option<TwoDimensionalRelation>,
}

fn editor_creator_authoring(contract: EditorCreatorContract) -> ParameterAuthoring {
    ParameterAuthoring::Creator(CreatorParameterMetadata {
        category: contract.category,
        unit: contract.unit,
        increment: contract.increment,
        precision: contract.precision,
        group: contract.group.into(),
        display_order: contract.display_order,
        ownership: contract.ownership,
        required: true,
        applicability: ParameterApplicability::Always,
        ...
        two_dimensional: contract.two_dimensional,
    })
}
...
fn pattern_editor_recipe(
    ...
    let definition_creator = |category,
                              unit,
                              increment,
                              precision,
                              group,
                              ...
                              invalidation,
                              two_dimensional| {
        editor_creator_authoring(EditorCreatorContract {
            category,
            unit,
            increment,
            precision,
    ...
    let channel_creator = |category,
        ...
        editor_creator_authoring(EditorCreatorContract {
            ...
            group,
            display_order,
            ownership: ParameterOwnership::ChannelInstance,
            invalidation,
            two_dimensional,
        })
    };
    ...
    let editor_parameters = vec![
        editor_text_parameter(
            ...
            definition_creator(
                ...
                "editor-curve",
                0,
                ParameterInvalidationScope::Geometry,
                None,
            ),
        ),
        ...
        editor_number_parameter(
            ...
            definition_creator(
                ...
                CreatorParameterIncrement::Number(1.0e-9),
                9,
                "editor-placement",
                1,
                ParameterInvalidationScope::Geometry,
        ...
        editor_number_parameter(
            "curve-spacing",
            "Curve Spacing",
            "Distance between repeated curve tracks.",
            draft.curve_spacing,
            EditorNumberBounds {
                minimum: 0.01,
                maximum: 100_000.0,
        ...
        editor_choice_parameter(
            ...
            definition_creator(
                ...
                "editor-placement",
                5,
                ParameterInvalidationScope::SourceSampling,
                None,
            ),
        ),
        editor_choice_parameter(
        ...
        editor_integer_parameter(
            ...
            channel_creator(
                ...
                CreatorParameterUnit::None,
                CreatorParameterIncrement::Integer(1),
                0,
                "editor-channel-sampling",
                1,
                ParameterInvalidationScope::SourceSampling,
    ...
    for (id, label, parameters) in [
        ...
        (
            "editor-curve",
            "Editor Curve",
            vec!["curve-path", "curve-function", "curve-spacing"],
        ),
        (
            "editor-deformation",
            "Editor Deformation",
            vec!["x-grid-curve", "y-grid-curve", "jitter-factor"],
        ),
        (
            "editor-output",
            "Editor Output",
            vec!["render-mode", "connection-mode"],
        ),
        (
            "editor-channel-sampling",
            "Editor Channel Sampling",
            vec!["point-sampler", "channel-seed", "channel-weight-influence"],
        ),
    ] {
        if !adapted
            .definition
        ...
        {
            adapted.definition.layout.sections.push(AuthoringSection {
                id: id.into(),
                label: label.into(),
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum PatternPresetAction {
    ApplyNamed(NamedPatternPreset),
    ActivateCurves,
    ActivateWeightedVoronoi,
    OpenCustomEditor,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct PatternPresetEntry {
    label: &'static str,
    action: PatternPresetAction,
}

const PATTERN_PRESET_ENTRIES: [PatternPresetEntry; 13] = [
    PatternPresetEntry {
        label: "X/Y Grid",
        action: PatternPresetAction::ApplyNamed(NamedPatternPreset::Grid),
    },
    PatternPresetEntry {
        label: "Triangular Grid (60°)",
        action: PatternPresetAction::ApplyNamed(NamedPatternPreset::TriangularGrid),
    },
    PatternPresetEntry {
        label: "Sine Wave",
        action: PatternPresetAction::ApplyNamed(NamedPatternPreset::SineWave),
    },
    PatternPresetEntry {
        label: "Square Wave",
        action: PatternPresetAction::ApplyNamed(NamedPatternPreset::SquareWave),
    },
    PatternPresetEntry {
        label: "Sawtooth",
        action: PatternPresetAction::ApplyNamed(NamedPatternPreset::Sawtooth),
    },
    PatternPresetEntry {
        label: "Random · Uniform",
        action: PatternPresetAction::ApplyNamed(NamedPatternPreset::RandomUniform),
    },
    PatternPresetEntry {
        label: "Random · Gaussian",
        action: PatternPresetAction::ApplyNamed(NamedPatternPreset::RandomGaussian),
    },
    PatternPresetEntry {
        label: "Random · Blue Noise",
        action: PatternPresetAction::ApplyNamed(NamedPatternPreset::RandomBlueNoise),
    },
    PatternPresetEntry {
        label: "Random · Pink Noise",
        action: PatternPresetAction::ApplyNamed(NamedPatternPreset::RandomPinkNoise),
    },
    PatternPresetEntry {
        label: "Random · Poisson",
        action: PatternPresetAction::ApplyNamed(NamedPatternPreset::RandomPoisson),
    },
    PatternPresetEntry {
        label: "Curves",
        action: PatternPresetAction::ActivateCurves,
    },
    PatternPresetEntry {
        label: "Weighted Voronoi",
        action: PatternPresetAction::ActivateWeightedVoronoi,
    },
    PatternPresetEntry {
        label: "Custom Pattern",
        action: PatternPresetAction::OpenCustomEditor,
    },
];

fn pattern_preset_entry(index: u32) -> Option<PatternPresetEntry> {
    PATTERN_PRESET_ENTRIES.get(index as usize).copied()
}

fn pattern_preset_index(action: PatternPresetAction) -> Option<u32> {
    PATTERN_PRESET_ENTRIES
        .iter()
        .position(|entry| entry.action == action)
        .map(|index| index as u32)
}

fn pattern_preset_labels() -> Vec<&'static str> {
    PATTERN_PRESET_ENTRIES
        .iter()
...
/// Artifact runs must end the application, not merely close the main window.
/// A modal or auxiliary GTK window can outlive the main window and keep the
/// non-unique CLI process alive after its requested artifact was written.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CliArtifactCompletion {
    KeepInteractive,
    QuitApplication,
}

fn cli_artifact_completion(recovery_enabled: bool) -> CliArtifactCompletion {
    if recovery_enabled {
        CliArtifactCompletion::KeepInteractive
    } else {
        CliArtifactCompletion::QuitApplication
    }
}

fn guided_catalog_initial_selection(
    catalog: &GuidedDefinitionCatalog,
    selected_definition: Option<&PatternId>,
) -> u32 {
    selected_definition
        .and_then(|id| {
            catalog
                .entries
                .iter()
                .position(|entry| entry.editable && &entry.id == id)
        })
        .or_else(|| catalog.entries.iter().position(|entry| entry.editable))
        .unwrap_or(0) as u32
}
...
enum DocumentLoadOutcome {
    Ready {
        document: Box<Document>,
        adjustments: toniator::persistence::LoadAdjustments,
    },
    MissingPatternDefinition {
        candidate: Box<MissingPatternDefinitionOpenCandidate>,
        source_path: PathBuf,
    },
}

/// Durable UI-only recovery state.  Re-presenting the dialog after a failed
/// chooser or recovery attempt keeps the exact decoded candidate and source
/// path alive without ever installing it into the editor.
struct MissingPatternDefinitionRecoverySession {
    candidate: Rc<MissingPatternDefinitionOpenCandidate>,
    source_path: PathBuf,
    feedback: RefCell<Option<String>>,
}
...
pub struct AppUi {
    ...
    guided_definition_catalog: gtk::Button,
    custom_pattern_edit: gtk::Button,
    definition_library: gtk::Button,
    ...
    channel_random_size_response: gtk::Scale,
    channel_random_size_response_status: gtk::Label,
...
struct ChannelControlWidgets {
    ...
    definition_parameter_slots: Vec<DefinitionChannelParameterSlot>,
}

#[derive(Clone)]
struct DefinitionChannelParameterSlot {
    row: gtk::Box,
    label: gtk::Label,
    spin: gtk::SpinButton,
...
fn refresh_guided_definition_widgets(
    draft: &GuidedDefinitionDraft,
    pipeline: &ArtworkPipelineSettings,
    picture: &gtk::Picture,
    feedback: &gtk::Label,
    controls: &[GuidedControlWidgets],
) {
    for widgets in controls {
        let applicable = draft.is_applicable(&widgets.parameter_id).unwrap_or(false);
        widgets.row.set_sensitive(applicable);
        let explanation =
            draft
                .control(&widgets.parameter_id)
                .and_then(|control| match &control.applicability {
                    toniator::ParameterApplicability::Always => None,
                    toniator::ParameterApplicability::WhenParameterEquals { parameter, value } => {
                        let label = draft
                            .control(parameter)
                            .map(|control| control.label.as_str())
                            .unwrap_or(parameter);
                        Some(format!("Available when {label} is {value:?}."))
                    }
                });
        widgets.availability.set_text(if applicable {
            ""
        } else {
            explanation
                .as_deref()
                .unwrap_or("Unavailable for this definition.")
        });
    }
    ...
    let result = draft
        ...
        .and_then(|output| {
            render_canonical_pattern_output_cancellable(&output, 480, 360, true, None, &token)
        });
    match result {
        Ok(image) => {
            ...
            feedback.set_text("Local canonical preview — changes are not applied until Apply.");
            feedback.remove_css_class("error");
        }
        Err(error) => {
            feedback.set_text(&format!("Preview validation: {error}"));
            feedback.add_css_class("error");
        }
    }
}

fn graph_layout_coordinate_accessibility(node_id: &str, axis: &str) -> (String, String) {
    (
        format!("{node_id} graph layout {axis}"),
        format!(
            "{axis} editor graph-layout coordinate for node `{node_id}`. This is not artwork geometry; changing it only changes the stored editor layout."
        ),
    )
}
...
fn build_channel_controls(channel: OutputChannelId) -> ChannelControlWidgets {
    ...
    let mut definition_parameter_slots = Vec::new();
    for _ in 0..8 {
        ...
        row.append(&label);
        row.append(&spin);
        content_host.append(&row);
        definition_parameter_slots.push(DefinitionChannelParameterSlot {
            row,
            label,
            spin,
...
fn output_channel_numeric_parameters(
    resolved: &toniator::ResolvedSelectedPatternDefinition,
) -> Vec<(&PatternParameterDefinition, &CreatorParameterMetadata)> {
    let mut parameters = resolved
        .definition
        .parameters
        .iter()
        .filter_map(|parameter| {
            ...
            (parameter.scope == DefinitionParameterScope::OutputChannel
                ...
                && matches!(
                    (parameter.value_type, metadata.category),
                    (
            ...
            .then_some((parameter, metadata))
        })
        .collect::<Vec<_>>();
    parameters.sort_by(|(left, left_meta), (right, right_meta)| {
        left_meta
            .group
            .cmp(&right_meta.group)
            .then(left_meta.display_order.cmp(&right_meta.display_order))
            .then(left.key.cmp(&right.key))
    });
    parameters
}

fn sync_definition_channel_parameter_slots(
    controls: &ChannelControlWidgets,
    resolved: Option<&toniator::ResolvedSelectedPatternDefinition>,
    sensitive: bool,
) {
    let parameters = resolved
        .map(output_channel_numeric_parameters)
        .unwrap_or_default();
    for (index, slot) in controls.definition_parameter_slots.iter().enumerate() {
        let Some((parameter, metadata)) = parameters.get(index).copied() else {
            slot.key.replace(None);
            slot.row.set_visible(false);
            continue;
        };
        let value = resolved
            .and_then(|resolved| {
                resolved
                    ...
                    .output_channel_values
                    .iter()
                    .find(|values| values.channel == controls.channel.stable_id())
                    .and_then(|values| {
                        values
                            .values
                            .iter()
                            .find(|value| value.key == parameter.key)
                    })
            })
            .map(|value| &value.value)
            .unwrap_or(&parameter.default);
        let (minimum, maximum, step, current, integer) = match (&parameter.constraints, value) {
            (
            ...
            | (
                PatternParameterConstraints::NumberLessThanParameter {
                    minimum,
                    maximum,
                    step,
                    ..
                ...
                LiteralValue::Number(value),
            ) => (*minimum, *maximum, *step, *value, false),
            (
        ...
        slot.key.replace(Some(parameter.key.clone()));
        slot.integer.set(integer);
        slot.label.set_text(&parameter.label);
        slot.label.set_tooltip_text(Some(&parameter.help));
        slot.spin.set_tooltip_text(Some(&parameter.help));
        slot.spin.update_property(&[
            gtk::accessible::Property::Label(&parameter.label),
            gtk::accessible::Property::Description(&parameter.help),
        ]);
        slot.spin.set_range(minimum, maximum);
        slot.spin
            .set_increments(step.max(if integer { 1.0 } else { 0.001 }), step.max(1.0));
        slot.spin.set_digits(if integer {
            0
        } else {
            u32::from(metadata.precision)
        });
        slot.spin.set_value(current);
        slot.spin.set_sensitive(sensitive);
        slot.row.set_visible(true);
    }
}
...
impl AppUi {
    pub fn new(application: &adw::Application, options: CliOptions) -> Rc<Self> {
        ...
        let ui = Rc::new(Self {
            ...
            guided_definition_catalog: editor_view.guided_definition_catalog.clone(),
            custom_pattern_edit: editor_view.custom_pattern_edit.clone(),
            definition_library: editor_view.definition_library.clone(),
            ...
            channel_random_size_response: editor_view.channel_random_size_response.clone(),
            channel_random_size_response_status: editor_view
                .channel_random_size_response_status
                .clone(),
        ...
        if load_example {
            ...
            if options.artifact_guided_spiral || options.artifact_guided_spiral_graph {
                ui.open_artifact_recipe_editor(
                    PatternId::QUADRATIC_RADIAL_SPIRAL_V1,
                    options.artifact_guided_spiral_graph,
                );
            } else if let Some(id) = options.artifact_guided_definition.as_deref() {
                match PatternId::new(id) {
                    Ok(id) => ui.open_artifact_recipe_editor(id, false),
                    Err(error) => {
                        ui.show_error(&format!("Invalid artifact definition ID: {error}"))
                    }
                }
            }
            if options.artifact_definition_library_project_copy {
                ui.install_artifact_project_copy();
            }
            if options.artifact_definition_library_user_conflict {
                ui.open_artifact_import_conflict(false);
            } else if options.artifact_definition_library_project_conflict {
                ui.open_artifact_import_conflict(true);
            } else if options.artifact_missing_definition_recovery {
                ui.open_artifact_missing_definition_recovery();
            } else if options.artifact_definition_library
                || options.artifact_definition_library_project_copy
            {
                ui.open_definition_library();
            }
    ...
    fn finish_cli_artifact_failure(&self, message: String) {
        ...
        self.finish_cli_artifact_session();
    }

    /// End a non-interactive artifact run even if a dialog remains open.
    ///
    /// Closing only `self.window` is insufficient: GTK keeps modal dialogs as
    /// application windows, so the application loop can stay alive after the
    /// screenshot was written. Quitting the registered application is generic
    /// and deliberately independent of which dialog, if any, supplied the
    /// capture surface.
    fn finish_cli_artifact_session(&self) {
        if cli_artifact_completion(self.recovery_enabled) != CliArtifactCompletion::QuitApplication
        {
            return;
        }
    ...
    fn connect_actions(
        ...
        self.pattern_preset.connect_selected_notify(glib::clone!(
            ...
            move |control| {
                ...
                match pattern_preset_entry(control.selected()).map(|entry| entry.action) {
                    Some(PatternPresetAction::ApplyNamed(preset)) => {
                        ui.apply_named_pattern_preset(preset)
                    }
                    Some(PatternPresetAction::ActivateCurves) => ui.activate_curve_treatment(),
                    Some(PatternPresetAction::ActivateWeightedVoronoi) => {
                        ui.activate_weighted_voronoi_treatment()
                    }
                    ...
                    Some(PatternPresetAction::OpenCustomEditor) => ui.open_pattern_editor(),
                    None => {}
                }
            }
        ));
        ...
        connect_clicked(&self.guided_definition_catalog, self, |ui| {
            ui.open_guided_definition_catalog()
        });
        connect_clicked(&self.custom_pattern_edit, self, |ui| {
            ui.open_pattern_editor()
        });
        connect_clicked(&self.definition_library, self, |ui| {
            ui.open_definition_library()
        });
        ...
        for controls in &self.channel_controls {
            ...
            for slot in &controls.definition_parameter_slots {
                let key = slot.key.clone();
                let integer = slot.integer.clone();
                slot.spin.connect_value_changed(glib::clone!(
                    #[weak(rename_to = ui)]
                    self,
                    move |control| {
                        if ui.state.borrow().syncing_controls {
                        ...
                        let value = if integer.get() {
                            LiteralValue::Integer(control.value().max(0.0).round() as u64)
                        } else {
                            LiteralValue::Number(control.value())
                        };
                        ui.change_selected_definition_channel_parameter(channel, &key, value);
                    }
                ));
            }
        }
        ...
        self.connect_web_scale(
            &self.channel_random_size_response,
    ...
    fn sync_pattern_editor_option_sensitivity(&self) {
        ...
        self.pattern_editor_y_grid_mode
            .set_sensitive(grid && !triangular);
        self.pattern_editor_x_spacing.set_sensitive(grid || math);
        ...
        self.pattern_editor_y_spacing
            .set_sensitive((grid && !triangular) || math);
        self.pattern_editor_x_grid_curve.set_sensitive(
            curve || math || (grid && !triangular && x_mode == PatternGridMode::Curve),
        );
        self.pattern_editor_y_grid_curve
            .set_sensitive(math || (grid && !triangular && y_mode == PatternGridMode::Curve));
        ...
        self.pattern_editor_curve_reset.set_sensitive(
            curve
                || math
                || (grid && (x_mode == PatternGridMode::Curve || y_mode == PatternGridMode::Curve)),
        );
    }
    ...
    fn pattern_editor_draft(&self) -> Result<Option<PatternEditorDraft>, String> {
        let state = self.state.borrow();
        let Some(editor) = state.editor.as_ref() else {
            return Ok(None);
        };
        ...
        let curve_function =
            pattern_curve_function_from_choice(&custom_choice("curve-function", "sine"))?;
        Ok(Some(PatternEditorDraft {
            name: custom
    ...
    fn apply_named_pattern_preset(&self, preset: NamedPatternPreset) {
        let mut draft = match self.pattern_editor_draft() {
            Ok(Some(draft)) => draft,
            Ok(None) => return,
            Err(error) => {
                self.show_error(&format!("Could not load Pattern Editor draft: {error}"));
                return;
            }
        };
        let entry = PATTERN_PRESET_ENTRIES
            .iter()
            .find(|entry| entry.action == PatternPresetAction::ApplyNamed(preset))
            .expect("named preset action must be present in the preset inventory");
        draft.name = entry.label.into();
        ...
        draft.render_mode = if matches!(
            preset,
            NamedPatternPreset::SineWave
                | NamedPatternPreset::SquareWave
                | NamedPatternPreset::Sawtooth
        ) {
            PatternRenderMode::ConnectedPoints
        } else {
            PatternRenderMode::Points
        };
        draft.point_definition = if matches!(
            preset,
            NamedPatternPreset::SineWave
                | NamedPatternPreset::SquareWave
                | NamedPatternPreset::Sawtooth
        ...
        match preset {
            NamedPatternPreset::Grid => draft.placement = PatternPlacement::Grid,
            NamedPatternPreset::TriangularGrid => {
                draft.placement = PatternPlacement::TriangularGrid
            }
            NamedPatternPreset::SineWave
            | NamedPatternPreset::SquareWave
            | NamedPatternPreset::Sawtooth => {
                draft.placement = PatternPlacement::MathFunction;
                draft.curve_function = match preset {
                    NamedPatternPreset::SineWave => PatternCurveFunction::Sine,
                    NamedPatternPreset::SquareWave => PatternCurveFunction::Square,
                    NamedPatternPreset::Sawtooth => PatternCurveFunction::Sawtooth,
                    _ => unreachable!("only math presets enter the math branch"),
                };
                // The remaining function presets visibly deform a grid even
                // when they start from neutral editor defaults.
            ...
            NamedPatternPreset::RandomUniform
            | NamedPatternPreset::RandomGaussian
            | NamedPatternPreset::RandomBlueNoise
            | NamedPatternPreset::RandomPinkNoise
            | NamedPatternPreset::RandomPoisson => {
                draft.placement = PatternPlacement::Random;
                draft.random_dispersion = match preset {
                    NamedPatternPreset::RandomUniform => PatternRandomDispersion::Uniform,
                    NamedPatternPreset::RandomGaussian => PatternRandomDispersion::Gaussian,
                    NamedPatternPreset::RandomBlueNoise => PatternRandomDispersion::BlueNoise,
                    NamedPatternPreset::RandomPinkNoise => PatternRandomDispersion::PinkNoise,
                    NamedPatternPreset::RandomPoisson => PatternRandomDispersion::Poisson,
                    _ => unreachable!("only random presets enter the random branch"),
                };
            }
        }
        eprintln!(
    ...
    /// Opens the registry-backed catalog for metadata-generated local Guided
    /// editors. This does not select or mutate a document pattern: bundled
    /// definitions remain immutable until Gate 5 adds explicit duplication and
    /// document-local Apply lifecycle.
    fn open_guided_definition_catalog(self: &Rc<Self>) {
        let registry = match load_bundled_pattern_definition_registry() {
            Ok(registry) => registry,
            Err(error) => {
                self.show_error(&format!("Could not load bundled pattern catalog: {error}"));
                return;
            }
        };
        ...
        let labels: Vec<_> = catalog
            ...
            .map(|entry| {
                if entry.editable {
                    ...
                    format!("{} — Guided unavailable", entry.name)
                }
            })
            .collect();
        ...
        dropdown.update_property(&[
            gtk::accessible::Property::Label("Bundled pattern definition"),
            gtk::accessible::Property::Description(
                "Registry-resolved immutable bundled definitions in stable ID order",
            ),
        ]);
        let selected_definition = self
            .state
            .borrow()
            .editor
            .as_ref()
            .and_then(|editor| editor.document().pattern_state.selected_pattern_id());
        dropdown.set_selected(guided_catalog_initial_selection(
            &catalog,
            selected_definition.as_ref(),
        ));
        let detail = gtk::Label::new(Some(
            "Guided controls are generated from strict metadata. Bundled definitions are immutable.",
        ));
        ...
        let dialog = adw::AlertDialog::builder()
            .heading("Guided Patterns")
            .body("Select a bundled definition to inspect its local schema-guided controls.")
            .build();
        dialog.add_responses(&[("cancel", "Cancel"), ("open", "Open Guided Editor")]);
        ...
        dialog.choose(
            ...
            glib::clone!(
                ...
                move |response| {
                    ...
                    let Some(id) = ids.get(dropdown.selected() as usize).cloned() else {
                        app_ui.show_error("The selected Guided definition is unavailable.");
                        return;
                    };
                    app_ui.open_bundled_recipe_editor(id);
                }
            ),
        );
    }

    fn next_document_local_recipe_id(&self, source: &PatternId) -> Result<PatternId, String> {
        let normalized = source.as_str().replace('.', "-");
        let state = self.state.borrow();
        let Some(editor) = state.editor.as_ref() else {
            return Err("There is no open document to receive a custom pattern.".into());
        };
        let patterns = &editor.document().pattern_state;
        for suffix in 1..=10_000_u32 {
            let candidate = PatternId::new(format!("custom.{normalized}.{suffix}"))
                .map_err(|error| error.to_string())?;
            if PATTERN_REGISTRY.get(candidate.clone()).is_none()
                && !patterns.instances.contains_key(&candidate)
                && !patterns
                    .bundled_definition_instances
                    .contains_key(&candidate)
                && !patterns.embedded_patterns.contains_key(&candidate)
            {
                return Ok(candidate);
            }
        }
        Err("Could not allocate a stable document-local pattern ID.".into())
    }

    fn open_definition_library(self: &Rc<Self>) {
        let resolver = {
            let state = self.state.borrow();
            let Some(editor) = state.editor.as_ref() else {
                return;
            ...
            match PatternDefinitionLifecycleResolver::load_for_document(
                ...
                Err(error) => {
                    self.show_error(&format!("Could not load the pattern library: {error}"));
                    return;
                }
            }
        };
        let dialog = adw::AlertDialog::builder()
            ...
            .body("Definitions are identified by stable ID. Bundled definitions stay immutable; using a user-library definition embeds its complete definition and creates validated defaults for this project's active output channels.")
            .build();
        ...
        feedback.update_property(&[
            gtk::accessible::Property::Label("Pattern library status"),
            gtk::accessible::Property::Description(
                "Reports explicit reload and project-embedding outcomes without changing the selected pattern on failure",
            ),
        ]);
        let reload = gtk::Button::with_label("Reload Library");
        reload.update_property(&[
            gtk::accessible::Property::Label("Reload Pattern Library"),
            gtk::accessible::Property::Description(
                "Reload direct .tnpattern files from the configured XDG user library",
            ),
        ]);
        let import = gtk::Button::with_label("Import Pattern…");
        import.update_property(&[
            gtk::accessible::Property::Label("Import external pattern definition"),
            gtk::accessible::Property::Description(
                "Strictly validates an external .tnpattern before offering explicit stable-ID conflict choices for the XDG user library",
            ),
        ]);
        let list = gtk::ListBox::new();
        list.set_selection_mode(gtk::SelectionMode::None);
        list.update_property(&[
            gtk::accessible::Property::Label("Pattern definition library"),
            gtk::accessible::Property::Description(
                "Bundled, user-library, and project-embedded pattern definitions in stable ID order",
            ),
        ]);
        ...
        dialog.set_extra_child(Some(&content));
        self.refresh_definition_library_rows(&resolver, &list, &feedback);
        import.connect_clicked(glib::clone!(
            ...
            #[weak]
            feedback,
            move |_| app_ui.open_external_pattern_import(&resolver, &list, &feedback)
        ));
        reload.connect_clicked(glib::clone!(
            ...
            move |_| {
                let result = {
                    let state = app_ui.state.borrow();
                    let Some(editor) = state.editor.as_ref() else {
                        return;
                    };
                    resolver
                        .borrow_mut()
                        .reload_for_document(&editor.document().pattern_state)
                };
                match result {
                    Ok(()) => {
                        feedback.set_text(
                            "Pattern library reloaded. Stable selection was not changed.",
                        );
                        feedback.remove_css_class("error");
                        app_ui.refresh_definition_library_rows(&resolver, &list, &feedback);
                    }
                    Err(error) => {
                        feedback.set_text(&format!(
                            "Reload failed; the previous library surface remains active: {error}"
                        ));
                        feedback.add_css_class("error");
                    }
                }
        ...
        let dialog_weak = dialog.downgrade();
        dialog.choose(Some(&self.window), None::<&gio::Cancellable>, move |_| {
            if let Some(dialog) = dialog_weak.upgrade() {
                dialog.set_extra_child(None::<&gtk::Widget>);
            }
        });
    ...
    fn open_external_pattern_import(
        self: &Rc<Self>,
        resolver: &Rc<RefCell<PatternDefinitionLifecycleResolver>>,
        list: &gtk::ListBox,
        feedback: &gtk::Label,
    ) {
        let dialog = gtk::FileDialog::builder()
            .title("Import Pattern Definition")
            .modal(true)
            .build();
        let filters = gio::ListStore::new::<gtk::FileFilter>();
        ...
        dialog.open(
            ...
            glib::clone!(
                ...
                move |result| {
                    ...
                    let Some(path) = file.path() else {
                        feedback.set_text("The selected import source has no local filesystem path.");
                        feedback.add_css_class("error");
                        return;
                    };
                    let plan = {
                        let resolver = resolver.borrow();
                        inspect_external_pattern_definition_import(&path, &resolver)
                    };
                    let plan = match plan {
                        Ok(plan) => plan,
                        Err(error) => {
                            feedback.set_text(&format!(
                                "Import rejected before changing the library: {error}"
                            ));
                            feedback.add_css_class("error");
                            return;
                        }
                    ...
                    match &plan {
                        ExternalPatternDefinitionImportPlan::Ready { .. } => {
                            app_ui.commit_external_pattern_import(
                                plan,
                        ...
                        ExternalPatternDefinitionImportPlan::Identical {
                            ...
                            feedback.set_text(&format!(
                                "Import deduplicated: {} is identical to the existing {} definition. The library was not changed.",
                                import.definition.id, existing_source
                            ));
                            feedback.remove_css_class("error");
                        }
                        ExternalPatternDefinitionImportPlan::UserLibraryConflict { .. }
                        | ExternalPatternDefinitionImportPlan::ProtectedConflict { .. } => {
                            app_ui.open_external_pattern_import_conflict(
                                plan,
                                &resolver,
                                &list,
                                &feedback,
    ...
    fn open_external_pattern_import_conflict(
        self: &Rc<Self>,
        ...
        let id = plan.import().definition.id.clone();
        let (heading, body, paths, allow_replace) = match &plan {
                ...
                if matching_paths.len() == 1 =>
            {
                (
                    "Pattern ID Conflict",
                    format!(
                        "The external definition uses stable ID {id}, which differs from one user-library file. Replacing that file is destructive; Cancel leaves the XDG library unchanged."
                    ),
                    matching_paths.clone(),
                    true,
                )
            }
            ExternalPatternDefinitionImportPlan::UserLibraryConflict { matching_paths, .. } => (
                "Duplicate User-Library IDs",
                format!(
                    "The external definition uses stable ID {id}, which is present in {} valid user-library files. No single-file replacement is safe; Cancel or import with a new custom stable ID.",
                    matching_paths.len(),
                ),
                matching_paths.clone(),
                false,
            ...
            ExternalPatternDefinitionImportPlan::ProtectedConflict {
                ...
                "Immutable Bundled Pattern ID",
                format!(
                    "The external definition uses stable ID {id}, which conflicts with immutable bundled content. Bundled content cannot be replaced; import only as a new custom stable ID or Cancel."
                ),
                Vec::new(),
                false,
            ),
            ExternalPatternDefinitionImportPlan::ProtectedConflict {
                ...
                "Project-Protected Pattern ID",
                format!(
                    "The external definition uses stable ID {id}, which is already owned by this project. Project content cannot be replaced by import. Use Edit Project Copy to change that project definition, or import this file with a new custom stable ID."
                ),
                Vec::new(),
                false,
            ),
            _ => unreachable!("only import conflicts reach this dialog"),
        };
        let dialog = adw::AlertDialog::builder()
            .heading(heading)
            .body(body)
        ...
        if allow_replace {
            dialog.add_responses(&[
                ("cancel", "Cancel"),
                ("replace", "Replace User-Library File (Destructive)"),
                ("new-id", "Import as New Custom ID"),
            ]);
        } else {
            dialog.add_responses(&[("cancel", "Cancel"), ("new-id", "Import as New Custom ID")]);
        }
        ...
        dialog.set_response_appearance("new-id", adw::ResponseAppearance::Suggested);
        let path_detail = if paths.is_empty() {
            String::new()
        } else if allow_replace {
            format!(
                "\nDestination that Replace will permanently overwrite:\n{}",
                paths[0].display()
            )
        } else {
            format!(
                "\nMatching user-library files; none will be replaced automatically:\n{}",
                paths
                    .iter()
        ...
        let detail = gtk::Label::new(Some(&format!(
            "External stable ID: {}{}\nGraph, schema, layout, assets, metadata, and definition defaults stay exact. Output-channel values are not imported because .tnpattern does not contain them.",
            plan.import().definition.id,
            path_detail,
        )));
        detail.set_wrap(true);
        detail.set_xalign(0.0);
        detail.update_property(&[
            gtk::accessible::Property::Label("Pattern import conflict details"),
            gtk::accessible::Property::Description(
                "Explains destructive replacement, duplicate user-library IDs, and protected bundled or project content without automatic replacement",
            ),
        ]);
        dialog.set_extra_child(Some(&detail));
        let dialog_weak = dialog.downgrade();
        dialog.choose(
            Some(&self.window),
            None::<&gio::Cancellable>,
            glib::clone!(
                ...
                move |response| {
                    ...
                    let choice = match response.as_str() {
                        "replace" => ExternalPatternDefinitionImportChoice::ReplaceUserLibrary,
                        "new-id" => ExternalPatternDefinitionImportChoice::ImportAsNewCustomId,
                        _ => return,
                    };
                    app_ui.commit_external_pattern_import(
                        plan.clone(),
    ...
    fn commit_external_pattern_import(
        self: &Rc<Self>,
        plan: ExternalPatternDefinitionImportPlan,
        ...
        let committed = {
            let resolver = resolver.borrow();
            commit_external_pattern_definition_import(&plan, choice, &resolver)
        };
        let committed = match committed {
            Ok(committed) => committed,
            Err(error) => {
                feedback.set_text(&format!(
                    "Import did not change the selected project or replace the destination: {error}"
                ));
                feedback.add_css_class("error");
                return;
            }
        ...
        if let Err(error) = reload {
            feedback.set_text(&format!(
                "Imported {} to {}, but reload failed; the previous library surface remains active and project selection is unchanged: {error}",
                committed.definition.id,
                committed.destination.display(),
            ));
            feedback.add_css_class("error");
            return;
        }
        feedback.set_text(&format!(
            "Imported {} to {}{} and reloaded the library. Project selection is unchanged.",
            committed.definition.id,
            committed.destination.display(),
            if committed.replaced_user_library_file {
                " by explicit user-library replacement"
            } else {
                ""
            }
        ));
    ...
    fn refresh_definition_library_rows(
        self: &Rc<Self>,
        resolver: &Rc<RefCell<PatternDefinitionLifecycleResolver>>,
        list: &gtk::ListBox,
        feedback: &gtk::Label,
        ...
        let (definitions, diagnostics) = {
            ...
            (
                resolver
                    ...
                    .cloned()
                    .collect::<Vec<_>>(),
                resolver.user_library().diagnostics().to_vec(),
            )
        };
        if diagnostics.is_empty() {
            feedback.set_text(
                "Library loaded. User-library definitions can be embedded into this project.",
            );
            feedback.remove_css_class("error");
        } else {
            feedback.set_text(&format!(
                "Library loaded with {} malformed or unreadable user file(s). Fix or remove this file, then Reload Library. Details: {}",
                diagnostics.len(),
                diagnostics
                    .iter()
        ...
        for resolved in definitions {
            ...
            let title = gtk::Label::new(Some(&resolved.definition.display.name));
            ...
            let detail = gtk::Label::new(Some(&format!(
                ...
                resolved.definition.id, resolved.authoritative_source, provenance
            )));
            detail.set_xalign(0.0);
            detail.set_wrap(true);
            detail.add_css_class("dim-label");
            ...
            if !resolved.diagnostics.is_empty() {
                let diagnostic = gtk::Label::new(Some(&format!(
                    "Resolution: {} user-library definition overridden by this project",
            ...
            if resolved.authoritative_source == PatternDefinitionSource::UserLibrary {
                let use_in_project = gtk::Button::with_label("Use in This Project");
                let pattern_id = resolved.definition.id.clone();
                use_in_project.update_property(&[
                    gtk::accessible::Property::Label(&format!(
                        "Use {} in this project",
                        pattern_id
                    )),
                    gtk::accessible::Property::Description(
                        "Embed this complete user-library definition with validated defaults for the project's active output channels as one undoable project edit",
                    ),
                ]);
                use_in_project.connect_clicked(glib::clone!(
                    #[weak(rename_to = app_ui)]
                    self,
                    #[strong]
                    resolver,
                    ...
                    feedback,
                    move |_| {
                        app_ui.use_user_library_definition(
                            pattern_id.clone(),
                            &resolver,
                            &list,
                            &feedback,
                ...
                content.append(&use_in_project);
            }
            if resolved.authoritative_source == PatternDefinitionSource::ProjectEmbedded {
                let edit_project_copy = gtk::Button::with_label("Edit Project Copy");
                let pattern_id = resolved.definition.id.clone();
                edit_project_copy.update_property(&[
                    gtk::accessible::Property::Label(&format!(
                        "Edit project copy {}",
                        pattern_id
                    )),
                    gtk::accessible::Property::Description(
                        "Open the complete embedded definition and its current instance in the shared Guided and Graph local draft. Apply is one undoable project edit; Cancel changes nothing.",
                    ),
                ]);
                edit_project_copy.connect_clicked(glib::clone!(
                    #[weak(rename_to = app_ui)]
                    self,
                    move |_| app_ui.open_project_embedded_recipe_editor(pattern_id.clone())
                ));
                content.append(&edit_project_copy);
            }
            row.set_child(Some(&content));
            row.update_property(&[gtk::accessible::Property::Label(&format!(
                "{} pattern definition {}",
                resolved.authoritative_source, resolved.definition.id
            ))]);
            list.append(&row);
        }
    }

    fn open_project_embedded_recipe_editor(self: &Rc<Self>, pattern_id: PatternId) {
        let embedded = {
            let state = self.state.borrow();
            let Some(editor) = state.editor.as_ref() else {
                return;
        ...
        let Some(embedded) = embedded else {
            self.show_error(&format!(
                "The stored project definition {pattern_id} is unavailable; the document was not changed."
            ));
            return;
        };
        let draft =
            match SharedRecipeEditorDraft::document_local(embedded.definition, embedded.instance) {
                Ok(draft) => draft,
                Err(error) => {
                    self.show_error(&format!(
                        "Could not reopen project definition {pattern_id} for editing: {error}"
                    ));
                    return;
                }
            };
        self.open_document_local_recipe_editor(draft, false, None);
    }

    fn use_user_library_definition(
        self: &Rc<Self>,
        ...
        let selection = {
            ...
            resolver.borrow().user_library_selection(
                &pattern_id,
                editor
                    .document()
                    .artwork_pipeline
        ...
        let (definition, instance) = match selection {
            Ok(selection) => selection,
            Err(error) => {
                feedback.set_text(&format!("Could not use {pattern_id}: {error}"));
                feedback.add_css_class("error");
                return;
            }
        };
        ...
        let Some(document) = document else {
            feedback.set_text(&format!(
                "Could not embed {pattern_id}; the project and current selection remain unchanged."
            ));
            feedback.add_css_class("error");
            return;
        };
        ...
        if let Err(error) = reload {
            feedback.set_text(&format!(
                "Embedded {pattern_id}, but library refresh failed; the project selection is preserved: {error}"
            ));
            feedback.add_css_class("error");
            return;
        }
        feedback.set_text(&format!(
            "Embedded and selected {pattern_id} as a portable project definition."
        ));
        feedback.remove_css_class("error");
        self.refresh_definition_library_rows(resolver, list, feedback);
    }

    /// The bundled definition is never modified. A user must explicitly make
    /// the complete document-local copy before any editor can Apply it.
    fn open_bundled_recipe_editor(self: &Rc<Self>, id: PatternId) {
        let channels = {
            let state = self.state.borrow();
            let Some(editor) = state.editor.as_ref() else {
                return;
        ...
        let resolved = match registry.get(&id) {
        ...
        let instance = match resolved.definition.default_instance_parameters(channels) {
        ...
        let draft = match SharedRecipeEditorDraft::bundled(resolved.definition.clone(), instance) {
            Ok(draft) => draft,
            Err(error) => {
                self.show_error(&error.to_string());
                return;
        ...
        let dialog = adw::AlertDialog::builder()
            .heading(format!("{} — Bundled", draft.definition().display.name))
            .body("Bundled definitions are immutable. Duplicate this complete recipe into the document before editing or applying it.")
            .build();
        dialog.add_responses(&[
            ("cancel", "Cancel"),
            ("duplicate", "Duplicate as Custom Pattern"),
        ]);
        dialog.set_default_response(Some("duplicate"));
        dialog.set_response_appearance("duplicate", adw::ResponseAppearance::Suggested);
        let detail = gtk::Label::new(Some(&format!(
            "Stable definition ID: {}\n{}\nThe duplicate retains recipe graph, layout, schema, assets, metadata, and instance values.",
            draft.definition().id,
            draft.definition().display.summary,
        )));
        detail.set_wrap(true);
        detail.set_xalign(0.0);
        dialog.set_extra_child(Some(&detail));
        ...
        dialog.choose(
            ...
            glib::clone!(
                ...
                move |response| {
                    ...
                    let custom_id = match app_ui.next_document_local_recipe_id(&id) {
                        Ok(id) => id,
                        Err(error) => {
                            app_ui.show_error(&error);
                            return;
                        }
                    };
                    match draft.duplicate_as_document_local(custom_id) {
                        Ok(draft) => app_ui.open_document_local_recipe_editor(draft, false, None),
                        Err(error) => app_ui.show_error(&error.to_string()),
                    }
                }
            ),
        );
    ...
    /// Artifact-only direct entry keeps the normal duplicate-first workflow
    /// intact while making the shared local editor inspectable without driving
    /// a pointer through a modal dialog. It never installs the draft.
    fn open_artifact_recipe_editor(self: &Rc<Self>, definition_id: PatternId, show_graph: bool) {
        let channels = {
            let state = self.state.borrow();
            let Some(editor) = state.editor.as_ref() else {
                return;
        ...
        let result = (|| {
            let registry = load_bundled_pattern_definition_registry()?;
            let resolved = registry.get(&definition_id)?;
            let instance = resolved.definition.default_instance_parameters(channels)?;
            let bundled = SharedRecipeEditorDraft::bundled(resolved.definition.clone(), instance)?;
            let id = PatternId::new(format!("custom.artifact-{}", definition_id.as_str()))?;
            bundled
                .duplicate_as_document_local(id)
                .map_err(anyhow::Error::from)
        })();
        match result {
            Ok(draft) => self.open_document_local_recipe_editor(draft, show_graph, None),
            Err(error) => self.show_error(&format!(
                "Could not prepare shared recipe editor artifact: {error}"
            )),
        }
    }

    /// Artifact-only setup for inspecting the generic project-copy row. It
    /// installs one ordinary embedded definition before the library opens and
    /// never changes the normal document-open or library workflow.
    fn install_artifact_project_copy(self: &Rc<Self>) {
        let prepared = (|| -> anyhow::Result<_> {
            ...
            let mut definition = registry
                ...
                .definition
                .clone();
            definition.id = PatternId::new("custom.artifact-project-copy.v1")?;
            definition.display.name = "Artifact Project Copy".into();
            definition.display.summary =
                "Isolated artifact project-embedded definition for Edit Project Copy.".into();
            let instance = definition.default_instance_parameters(channels)?;
            Ok((definition, instance))
        })();
        let (definition, instance) = match prepared {
            Ok(prepared) => prepared,
            Err(error) => {
                self.show_error(&format!("Could not prepare project-copy artifact: {error}"));
                return;
            }
        };
        let document = {
        ...
        if let Some(document) = document {
            ...
            self.show_error("Could not install the project-copy artifact definition.");
        }
    }

    /// Artifact-only conflict entrypoint. The dialog uses the same typed import
    /// plans as production; it does not write a source or destination file.
    fn open_artifact_import_conflict(self: &Rc<Self>, project_protected: bool) {
        let definition = match (|| -> anyhow::Result<_> {
            let registry = load_bundled_pattern_definition_registry()?;
            let mut definition = registry
                .get(&PatternId::QUADRATIC_RADIAL_SPIRAL_V1)?
                .definition
                .clone();
            definition.id = PatternId::new(if project_protected {
                "custom.artifact-project-import-conflict.v1"
            } else {
                "custom.artifact-user-import-conflict.v1"
            })?;
            definition.display.name = "Artifact Import Conflict".into();
            Ok(definition)
        })() {
            Ok(definition) => definition,
            Err(error) => {
                self.show_error(&format!(
                    "Could not prepare import-conflict artifact: {error}"
                ));
                return;
            }
        };
        let mut incoming = definition.clone();
        incoming.display.name = "Incoming Artifact Import".into();
        let import = ExternalPatternDefinitionImport {
            source_path: native_user_pattern_library_dir().join("external-source.tnpattern"),
            definition: incoming,
        };
        let plan = if project_protected {
            ExternalPatternDefinitionImportPlan::ProtectedConflict {
                import,
                source: PatternDefinitionSource::ProjectEmbedded,
            }
            ...
            ExternalPatternDefinitionImportPlan::UserLibraryConflict {
                ...
                matching_paths: vec![
                    native_user_pattern_library_dir()
                        .join("artifact-existing-definition.tnpattern"),
                ],
            }
        };
        let resolver = {
            ...
            match PatternDefinitionLifecycleResolver::load_for_document(
                ...
                Ok(resolver) => Rc::new(RefCell::new(resolver)),
                Err(error) => {
                    self.show_error(&format!(
                        "Could not prepare import-conflict artifact: {error}"
                    ));
        ...
        let list = gtk::ListBox::new();
        let feedback = gtk::Label::new(None);
        self.open_external_pattern_import_conflict(plan, &resolver, &list, &feedback);
    }

    /// Artifact-only missing-authority entrypoint.  It creates an isolated
    /// current-schema fixture in a temporary directory, then uses the same
    /// persistence candidate and native recovery dialog as a real open.  The
    /// temporary source is never installed into the editor or written back.
    fn open_artifact_missing_definition_recovery(self: &Rc<Self>) {
        let prepared = (|| -> anyhow::Result<_> {
            ...
            let temporary = std::env::temp_dir().join(format!(
                "toniator-missing-definition-artifact-{}-{nonce}",
                std::process::id()
            ));
            std::fs::create_dir(&temporary)?;
            let path = temporary.join("missing-definition.toniator");
            let mut definition = toniator::load_bundled_shapes_definition()?;
            definition.id = PatternId::new("custom.artifact-missing-definition.v1")?;
            definition.display.name = "Missing Artifact Definition".into();
            definition.display.summary = "An isolated recovery screenshot fixture.".into();
            let instance = definition.default_instance_parameters(
            ...
            let document = self
                ...
                .ok_or_else(|| {
                    anyhow::anyhow!("missing-definition artifact requires the example document")
                })?;
            let mut value: serde_json::Value =
                serde_json::from_slice(&toniator::persistence::document_json(&document)?)?;
            ...
            let candidate = match toniator::persistence::load_document_open_candidate_with_library(
                &path,
                temporary.join("library"),
            )? {
                DocumentOpenCandidate::MissingPatternDefinition(candidate) => *candidate,
                DocumentOpenCandidate::Valid(_) => {
                    anyhow::bail!("artifact fixture did not require recovery")
                }
            };
            // The candidate owns only decoded values; no production action is
            // taken from this artifact prompt, so its source fixture can be
            // removed immediately after the dialog input is prepared.
            std::fs::remove_dir_all(&temporary)?;
            Ok((candidate, path))
        })();
        match prepared {
            Ok((candidate, path)) => {
                let session = Rc::new(MissingPatternDefinitionRecoverySession {
                    candidate: Rc::new(candidate),
                    source_path: path,
                    feedback: RefCell::new(Some(
                        "Matching recovery did not change the document: select a current .tnpattern with the requested stable ID. Recovery is still pending.".into(),
                    )),
                });
                self.present_missing_pattern_definition_recovery(session);
            }
            Err(error) => self.show_error(&format!(
                "Could not prepare missing-definition recovery artifact: {error:#}"
            )),
        }
    }

    fn open_document_local_recipe_editor(
        self: &Rc<Self>,
        draft: SharedRecipeEditorDraft,
        show_graph: bool,
        ...
        let dialog = adw::AlertDialog::builder()
            .heading(format!("{} — Custom Pattern", draft.definition().display.name))
            .body("Guided and Graph edit one local recipe draft. Apply commits it atomically; Cancel leaves the document unchanged.")
            .build();
        ...
        preview.set_size_request(420, 260);
        preview.update_property(&[
            gtk::accessible::Property::Label("Custom pattern local canonical preview"),
            gtk::accessible::Property::Description(
                "Canonical preview of the shared local recipe draft. It changes only after a Guided value edit, not after Graph layout positioning.",
            ),
        ]);
        let feedback = gtk::Label::new(None);
        feedback.set_wrap(true);
        feedback.set_xalign(0.0);
        let controls = Rc::new(RefCell::new(Vec::<GuidedControlWidgets>::new()));
        let guided = gtk::Box::new(gtk::Orientation::Vertical, 10);
        guided.append(&preview);
        guided.append(&feedback);
        for section in draft.borrow().sections() {
            let heading = gtk::Label::new(Some(&section.label));
            heading.set_xalign(0.0);
            heading.add_css_class("heading");
            guided.append(&heading);
            for descriptor in &section.controls {
                let row = gtk::Box::new(gtk::Orientation::Vertical, 4);
                let label = gtk::Label::new(Some(&descriptor.label));
                label.set_xalign(0.0);
                let help = gtk::Label::new(Some(&descriptor.help));
                help.set_xalign(0.0);
                help.set_wrap(true);
                help.add_css_class("dim-label");
                let availability = gtk::Label::new(None);
                availability.set_xalign(0.0);
                availability.set_wrap(true);
                availability.add_css_class("dim-label");
                row.append(&label);
                let parameter_id = descriptor.parameter_id.clone();
                let widget: gtk::Widget = match descriptor.widget {
                    GuidedWidgetKind::Toggle => {
                        let toggle = gtk::CheckButton::new();
                        toggle.set_active(matches!(
                            draft.borrow().value(&parameter_id),
                            Some(LiteralValue::Boolean(true))
                        ));
                        let state = draft.clone();
                        let picture = preview.clone();
                        let status = feedback.clone();
                        let widgets = controls.clone();
                        let pipeline = pipeline.clone();
                        let parameter = parameter_id.clone();
                        toggle.connect_toggled(move |toggle| {
                            if let Err(error) = state
                                .borrow_mut()
                                .set_value(&parameter, LiteralValue::Boolean(toggle.is_active()))
                            {
                            ...
                            refresh_guided_definition_widgets(
                                shared.guided(),
                                &pipeline,
                                &picture,
                                &status,
                    ...
                    GuidedWidgetKind::Enumeration => {
                        let choices = descriptor.choices.clone();
                        let refs: Vec<_> = choices.iter().map(String::as_str).collect();
                        let dropdown = gtk::DropDown::new(
                            Some(gtk::StringList::new(&refs)),
                            None::<&gtk::Expression>,
                        );
                        if let Some(LiteralValue::Choice(value)) =
                            draft.borrow().value(&parameter_id)
                        ...
                        dropdown.connect_selected_notify(move |dropdown| {
                            let Some(choice) = choices.get(dropdown.selected() as usize) else {
                                return;
                            };
                            if let Err(error) = state
                                .borrow_mut()
                                .set_value(&parameter, LiteralValue::Choice(choice.clone()))
                            {
                                status.set_text(&error.to_string());
                                status.add_css_class("error");
                                return;
                    ...
                    _ => {
                        let numeric = descriptor
                            .numeric
                            .as_ref()
                            .expect("numeric widget requires metadata")
                            .clone();
                        ...
                        spin.set_digits(numeric.digits.into());
                        spin.set_value(match draft.borrow().value(&parameter_id) {
                            Some(LiteralValue::Number(value)) => numeric.to_display(*value),
                            Some(LiteralValue::Integer(value)) => *value as f64,
                            _ => numeric.minimum,
                        });
                        let integer = matches!(descriptor.widget, GuidedWidgetKind::Integer);
                        let state = draft.clone();
                        let picture = preview.clone();
                        let status = feedback.clone();
                        let widgets = controls.clone();
                        ...
                        spin.connect_value_changed(move |spin| {
                            let value = if integer {
                                LiteralValue::Integer(spin.value_as_int().max(0) as u64)
                            } else {
                                LiteralValue::Number(numeric.from_display(spin.value()))
                            };
                            if let Err(error) = state.borrow_mut().set_value(&parameter, value) {
                                status.set_text(&error.to_string());
                                status.add_css_class("error");
                                return;
                            }
                ...
                widget.update_property(&[
                    gtk::accessible::Property::Label(&descriptor.label),
                    gtk::accessible::Property::Description(&descriptor.help),
                ]);
                widget.update_relation(&[
                    gtk::accessible::Relation::LabelledBy(&[label.upcast_ref()]),
                    gtk::accessible::Relation::DescribedBy(&[
                        help.upcast_ref(),
                        availability.upcast_ref(),
                    ]),
                ]);
                row.append(&widget);
                row.append(&help);
                ...
                guided.append(&row);
                controls.borrow_mut().push(GuidedControlWidgets {
                    parameter_id,
                    row,
                    availability,
                });
        ...
        let graph = gtk::Box::new(gtk::Orientation::Vertical, 10);
        let graph_intro = gtk::Label::new(Some(
            "Bounded Graph shows stored nodes and edges. X/Y are editor layout coordinates—not artwork geometry or pixels. Moving them changes only editor layout, never the canonical preview. Topology, arguments, assets, and schema stay verbatim.",
        ));
        graph_intro.set_xalign(0.0);
        graph_intro.set_wrap(true);
        graph.append(&graph_intro);
        for node in &draft.borrow().definition().recipe.nodes {
            let position = *draft
                .borrow()
                .definition()
                .layout
                .node_positions
                .get(&node.id)
                .expect("validated recipe graph must position each node");
            let row = gtk::Box::new(gtk::Orientation::Horizontal, 8);
            let label = gtk::Label::new(Some(&format!(
                "{} — {} v{}",
                node.id, node.operation.id, node.operation.version
            )));
            label.set_hexpand(true);
            label.set_xalign(0.0);
            let x = gtk::SpinButton::with_range(-10_000.0, 10_000.0, 1.0);
            let y = gtk::SpinButton::with_range(-10_000.0, 10_000.0, 1.0);
            x.set_value(position.x.into());
            y.set_value(position.y.into());
            let x_label = gtk::Label::new(Some("X"));
            let y_label = gtk::Label::new(Some("Y"));
            let (x_accessible_name, x_accessible_description) =
                graph_layout_coordinate_accessibility(&node.id, "X");
            let (y_accessible_name, y_accessible_description) =
                graph_layout_coordinate_accessibility(&node.id, "Y");
            x.update_property(&[
                gtk::accessible::Property::Label(&x_accessible_name),
                gtk::accessible::Property::Description(&x_accessible_description),
            ]);
            y.update_property(&[
                gtk::accessible::Property::Label(&y_accessible_name),
                gtk::accessible::Property::Description(&y_accessible_description),
            ]);
            x.update_relation(&[gtk::accessible::Relation::LabelledBy(&[
                label.upcast_ref(),
                x_label.upcast_ref(),
            ])]);
            y.update_relation(&[gtk::accessible::Relation::LabelledBy(&[
                label.upcast_ref(),
                y_label.upcast_ref(),
            ])]);
            row.append(&label);
            ...
            for changed in [&x, &y] {
                ...
                let y = y.clone();
                let status = feedback.clone();
                changed.connect_value_changed(move |_| {
                    if let Err(error) = state.borrow_mut().set_node_position(
                        &node_id,
                        GraphPosition {
                            x: x.value() as f32,
                            y: y.value() as f32,
                    ...
                    status.set_text(
                        "Graph layout updated locally. Editor graph layout does not change the canonical preview; Apply commits this shared draft.",
                    );
                    status.remove_css_class("error");
                });
            }
        }
        let edges = gtk::Label::new(Some(&format!(
            "Edges: {}",
            draft
                .borrow()
                .definition()
        ...
        edges.set_wrap(true);
        edges.set_xalign(0.0);
        edges.add_css_class("dim-label");
        graph.append(&edges);
        let stack = gtk::Stack::new();
        stack.add_titled(&guided, Some("guided"), "Guided");
        ...
        let draft_status = gtk::Label::new(Some(&format!(
            "Local draft ID: {} · Inherited name: “{}”. Save Draft writes this complete definition with current structural defaults to the XDG library without applying it; per-channel values stay project-local.",
            draft.borrow().definition().id,
            draft.borrow().definition().display.name,
        )));
        draft_status.set_wrap(true);
        draft_status.set_xalign(0.0);
        draft_status.add_css_class("dim-label");
        let save_as = gtk::Button::with_label("Save Draft to Library…");
        save_as.set_halign(gtk::Align::Start);
        save_as.update_property(&[
            gtk::accessible::Property::Label("Save complete custom pattern draft to library"),
            gtk::accessible::Property::Description(
                "Writes the complete definition with its current definition-owned structural values promoted to defaults. Per-channel instance values are not copied to the XDG library. The project is not changed.",
            ),
        ]);
        ...
        save_as.connect_clicked(glib::clone!(
            #[weak(rename_to = app_ui)]
            self,
            move |_| app_ui.save_shared_recipe_draft_as(save_draft.clone(), save_feedback.clone())
        ));
        let content = gtk::Box::new(gtk::Orientation::Vertical, 8);
        content.append(&draft_status);
        ...
        if let Some(message) = initial_feedback {
        ...
        dialog.choose(
            ...
            glib::clone!(
                ...
                move |response| {
                    ...
                    let shared = draft.borrow();
                    if let Err(error) = shared.validate_for_apply() {
                        let retry = shared.clone();
                        drop(shared);
                        app_ui.open_document_local_recipe_editor(
                            retry,
                            show_graph,
                            Some(format!(
                                "Apply rejected: {error}. The exact local draft was retained; correct it and Apply again."
                            )),
                        );
                        return;
                    }
                    ...
                    drop(shared);
                    let applied = {
                        let mut state = app_ui.state.borrow_mut();
                        state.editor.as_mut().is_some_and(|editor| {
                            editor.install_and_select_embedded_pattern(definition, instance)
                        })
                    };
                    if !applied {
                        let retry = draft.borrow().clone();
                        app_ui.open_document_local_recipe_editor(
                            retry,
                            show_graph,
                            Some(
                                "Apply rejected by document validation. The exact local draft was retained; correct it and Apply again. The document remains unchanged."
                                    .into(),
                            ),
                        );
                        return;
                    }
                    app_ui.sync_controls();
                }
            ),
        );
    }

    fn save_shared_recipe_draft_as(
        self: &Rc<Self>,
        draft: Rc<RefCell<SharedRecipeEditorDraft>>,
        feedback: gtk::Label,
    ) {
        let directory = native_user_pattern_library_dir();
        if let Err(error) = std::fs::create_dir_all(&directory) {
            feedback.set_text(&format!(
                "Could not create the XDG pattern library: {error}"
            ));
            feedback.add_css_class("error");
            return;
        }
        let definition = draft.borrow().definition().clone();
        let dialog = gtk::FileDialog::builder()
            .title("Save Complete Pattern Draft to Library")
            .modal(true)
            .initial_folder(&gio::File::for_path(&directory))
            .initial_name(format!(
                "{}.tnpattern",
                pattern_id_component(&definition.display.name)
            ))
            .build();
        let filters = gio::ListStore::new::<gtk::FileFilter>();
        let patterns = gtk::FileFilter::new();
        ...
        dialog.save(
            ...
            glib::clone!(
                ...
                move |result| {
                    ...
                    let Some(path) = file.path() else {
                        feedback.set_text("The selected library destination has no local filesystem path.");
                        feedback.add_css_class("error");
                        return;
                    };
                    let path = normalized_pattern_path(&path);
                    if path.parent() != Some(directory.as_path()) {
                        feedback.set_text(&format!(
                            "Save As only writes direct files in the XDG pattern library: {}",
                            directory.display()
                        ));
                        feedback.add_css_class("error");
                        return;
                    ...
                    let definition = {
                        let shared = draft.borrow();
                        project_definition_for_library_save(shared.definition(), shared.instance())
                    };
                    let definition = match definition {
                        Ok(definition) => definition,
                        Err(error) => {
                            feedback.set_text(&format!(
                                "Could not project the draft's structural defaults for library save; the local draft and project remain unchanged: {error}"
                            ));
                            feedback.add_css_class("error");
                            return;
                        }
                    };
                    if let Err(error) = save_user_pattern_definition(&path, &definition) {
                        feedback.set_text(&format!(
                            "Could not save the complete draft; the local draft and project remain unchanged: {error}"
                        ));
                        feedback.add_css_class("error");
                        return;
                    }
                    let (selected_before, reload) = {
                        let state = app_ui.state.borrow();
                        let Some(editor) = state.editor.as_ref() else {
                            return;
                        };
                        (
                            ...
                            PatternDefinitionLifecycleResolver::load_for_document(
                                directory.clone(),
                                &editor.document().pattern_state,
                            )
                    ...
                    match reload.and_then(|resolver| {
                        resolver
                            .resolve(&definition.id)
                            .map_err(|diagnostic| anyhow::anyhow!(
                                "saved definition {} was not discoverable: {:?}",
                                definition.id,
                                diagnostic
                            ))
                            .map(|_| ())
                        ...
                        Ok(_) => {
                            let selection_unchanged = app_ui
                                .state
                                .borrow()
                                .editor
                                .as_ref()
                                ...
                                == selected_before;
                            feedback.set_text(&format!(
                                "Saved complete definition with current structural defaults to {} and reloaded library discovery. Stable project selection {}.",
                                path.display(),
                                if selection_unchanged { "is unchanged" } else { "changed unexpectedly" }
                            ));
                        ...
                        Err(error) => {
                            feedback.set_text(&format!(
                                "Draft was written to {}, but library reload failed; the draft and project selection remain unchanged: {error}",
                                path.display()
                            ));
                            feedback.add_css_class("error");
                        }
    ...
    #[allow(dead_code)]
    fn open_guided_definition_editor(self: &Rc<Self>, id: PatternId) {
        let (pipeline, channels) = {
            let state = self.state.borrow();
            let Some(editor) = state.editor.as_ref() else {
                return;
            };
        ...
        let catalog = GuidedDefinitionCatalog::from_registry(&registry);
        let draft = match catalog.open(&registry, &id, channels) {
            Ok(draft) => draft,
            Err(error) => {
                self.show_error(&error.to_string());
                return;
        ...
        let title = format!("{} — Guided", draft.definition().display.name);
        let dialog = adw::AlertDialog::builder()
            .heading(&title)
            .body("Bundled definitions are immutable. Changes update this local preview only; duplicate to a document-local copy is required before Apply (Gate 5).")
            .build();
        dialog.add_response("close", "Close");
        dialog.set_close_response("close");
        ...
        preview.set_size_request(420, 260);
        preview.update_property(&[
            gtk::accessible::Property::Label("Guided definition local canonical preview"),
            gtk::accessible::Property::Description(
                "Preview rendered from the local immutable definition draft through its recipe graph",
            ),
        ]);
        let feedback = gtk::Label::new(None);
        feedback.set_wrap(true);
        ...
        for section in draft.borrow().sections() {
            ...
            for descriptor in &section.controls {
                ...
                let suffix = descriptor
                    ...
                    .unwrap_or("");
                if !suffix.is_empty() {
                    label.set_text(&format!("{} ({suffix})", descriptor.label));
                }
                let help = gtk::Label::new(Some(&descriptor.help));
                help.set_xalign(0.0);
                help.set_wrap(true);
                ...
                availability.set_wrap(true);
                availability.add_css_class("dim-label");
                availability.add_css_class("caption");
                row.append(&label);
                ...
                let widget: gtk::Widget = match descriptor.widget {
                    ...
                    GuidedWidgetKind::Enumeration => {
                        let choice_refs: Vec<_> =
                            descriptor.choices.iter().map(String::as_str).collect();
                        let choice_model = gtk::StringList::new(&choice_refs);
                        let dropdown =
                            gtk::DropDown::new(Some(choice_model), None::<&gtk::Expression>);
                        let current = draft.borrow().value(&parameter_id).cloned();
                        if let Some(LiteralValue::Choice(value)) = current
                    ...
                    _ => {
                        ...
                        spin.set_digits(numeric.digits.into());
                        let value = match draft.borrow().value(&parameter_id) {
                            Some(LiteralValue::Number(value)) => numeric.to_display(*value),
                            Some(LiteralValue::Integer(value)) => *value as f64,
                            _ => numeric.minimum,
                        };
    ...
    fn open_pattern_editor(self: &Rc<Self>) {
        let draft = match self.pattern_editor_draft() {
            Ok(Some(draft)) => draft,
            Ok(None) => return,
            Err(error) => {
                self.show_error(&format!("Could not open Pattern Editor: {error}"));
                return;
            }
        };
        let dialog = adw::AlertDialog::builder()
    ...
    fn load_candidate_async(self: &Rc<Self>, path: PathBuf, is_document: bool, recovered: bool) {
        ...
        std::thread::spawn(move || {
            let candidate = (|| -> anyhow::Result<DocumentLoadOutcome> {
                if is_document {
                    match toniator::persistence::load_document_open_candidate(&path_for_worker)? {
                        DocumentOpenCandidate::Valid(loaded) => {
                            toniator::render::decode_source(&loaded.document.source, 128)?;
                            Ok(DocumentLoadOutcome::Ready {
                                document: Box::new(loaded.document),
                                adjustments: loaded.adjustments,
                            })
                        }
                        DocumentOpenCandidate::MissingPatternDefinition(candidate) => {
                            Ok(DocumentLoadOutcome::MissingPatternDefinition {
                                candidate,
                                source_path: path_for_worker.clone(),
                            })
                        }
                    }
                } else {
                    ...
                    Ok(DocumentLoadOutcome::Ready {
                        document: Box::new(Document::new_with_artboard(source, width, height)),
                ...
                move || {
                    ...
                    match candidate {
                        ...
                        Ok(DocumentLoadOutcome::MissingPatternDefinition {
                            candidate,
                            source_path,
                        }) => ui.open_missing_pattern_definition_recovery(*candidate, source_path),
                        Err(error) => {
                            let message = format!(
                            ...
                            if ui.deferred_candidate_artifact {
                                ...
                                ui.finish_cli_artifact_session();
                            }
                        }
                    }
                    glib::ControlFlow::Break
    ...
    /// Presents recovery without ever installing the invalid decoded document.
    /// Both actions return a complete, freshly validated candidate before the
    /// ordinary dirty-transition gate is allowed to replace the active editor.
    fn open_missing_pattern_definition_recovery(
        self: &Rc<Self>,
        candidate: MissingPatternDefinitionOpenCandidate,
        source_path: PathBuf,
    ) {
        let session = Rc::new(MissingPatternDefinitionRecoverySession {
            candidate: Rc::new(candidate),
            source_path,
            feedback: RefCell::new(None),
        });
        self.present_missing_pattern_definition_recovery(session);
    }

    fn present_missing_pattern_definition_recovery(
        self: &Rc<Self>,
        session: Rc<MissingPatternDefinitionRecoverySession>,
    ) {
        let diagnostic = session.candidate.diagnostic().clone();
        let replacement_candidates = session.candidate.replacement_candidates().to_vec();
        let has_replacements = !replacement_candidates.is_empty();
        let detail = gtk::Label::new(Some(&format!(
            "Requested stable ID: {}\nAvailable replacement definitions: {}\n\nLocate Matching requires a current valid .tnpattern with exactly the requested stable ID. A replacement discards incompatible values and creates fresh defaults; old structural values cannot be mapped.",
            ...
            if replacement_candidates.is_empty() {
                ...
                replacement_candidates
                    .iter()
                    .map(missing_pattern_replacement_label)
                    .collect::<Vec<_>>()
                    .join("\n")
            },
        )));
        ...
        detail.set_xalign(0.0);
        detail.update_property(&[
            gtk::accessible::Property::Label("Missing pattern definition recovery details"),
            gtk::accessible::Property::Description(
                "Shows the requested stable ID, creator-facing replacement name and provenance, and the consequences of exact recovery or destructive replacement",
            ),
        ]);
        let replacement = has_replacements.then(|| {
            let labels = replacement_candidates
                .iter()
                .map(missing_pattern_replacement_label)
                .collect::<Vec<_>>();
            let refs = labels.iter().map(String::as_str).collect::<Vec<_>>();
            let dropdown = gtk::DropDown::from_strings(&refs);
            dropdown.set_tooltip_text(Some("Choose a replacement stable pattern ID"));
            dropdown.update_property(&[
                gtk::accessible::Property::Label("Replacement stable pattern ID"),
                gtk::accessible::Property::Description(
                    "Creator-facing name and provenance are displayed, while stable ID remains the replacement authority. This action creates a fresh default instance and discards incompatible old structural values",
                ),
            ]);
            dropdown
        });
        ...
        if let Some(feedback_detail) = session.feedback.borrow().as_deref() {
            let feedback_summary = missing_pattern_recovery_feedback_summary(feedback_detail);
            let feedback = gtk::Label::new(Some(feedback_summary));
            feedback.set_wrap(true);
            feedback.set_xalign(0.0);
            feedback.set_tooltip_text(Some(feedback_detail));
            feedback.add_css_class("error");
            feedback.update_property(&[
                gtk::accessible::Property::Label("Recovery feedback"),
                gtk::accessible::Property::Description(&format!(
                    "{} Technical recovery detail: {feedback_detail}",
                    feedback_summary
                )),
            ]);
            content.append(&feedback);
        ...
        let dialog = adw::AlertDialog::builder()
            .heading("Missing Pattern Definition")
            .body(format!(
                "This current Toniator document selects {}, but its definition authority is unavailable. Cancel leaves the current document and source file unchanged.",
                diagnostic.requested_id
            ))
            .build();
        let mut responses = vec![
            ("cancel", MISSING_PATTERN_RECOVERY_CANCEL_RESPONSE),
            ("locate", MISSING_PATTERN_RECOVERY_LOCATE_RESPONSE),
        ];
        if has_replacements {
            responses.push(("replacement", MISSING_PATTERN_RECOVERY_REPLACEMENT_RESPONSE));
        }
        ...
        if has_replacements {
            dialog.set_response_appearance("replacement", adw::ResponseAppearance::Destructive);
        }
        dialog.set_extra_child(Some(&content));
        let dialog_weak = dialog.downgrade();
        dialog.choose(
            ...
            glib::clone!(
                ...
                move |response| {
                    ...
                    match response.as_str() {
                        "locate" => ui.locate_missing_pattern_definition(session),
                        "replacement" => {
                            let Some(dropdown) = replacement else {
                                return;
                            };
                            let Some(candidate) =
                                replacement_candidates.get(dropdown.selected() as usize)
                            else {
                                return;
                            };
                            match session.candidate.recover_with_replacement(&candidate.id) {
                                Ok(loaded) => ui.install_recovered_missing_pattern_document(
                                    loaded,
                                    session,
                                ),
                                Err(error) => ui.retry_missing_pattern_definition_recovery(
                                    session,
                                    format!(
                                        "Replacement recovery did not change the document: {error:#}"
                                    ),
                                ),
                            }
                        }
    ...
    fn locate_missing_pattern_definition(
        self: &Rc<Self>,
        session: Rc<MissingPatternDefinitionRecoverySession>,
    ) {
        let dialog = gtk::FileDialog::builder()
            .title("Locate Matching Pattern Definition")
            .modal(true)
            .build();
        let filters = gio::ListStore::new::<gtk::FileFilter>();
        let patterns = gtk::FileFilter::new();
        ...
        dialog.open(
            ...
            glib::clone!(
                ...
                move |result| {
                    let Ok(file) = result else {
                        ui.retry_missing_pattern_definition_recovery(
                            session,
                            "No matching definition was selected. Recovery is still pending.",
                        );
                        return;
                    };
                    let Some(path) = file.path() else {
                        ui.retry_missing_pattern_definition_recovery(
                            session,
                            "Recovery source has no local filesystem path. Choose a local .tnpattern file.",
                        );
                        return;
                    };
                    match session.candidate.recover_with_matching_file(&path) {
                        Ok(loaded) => {
                            ui.install_recovered_missing_pattern_document(loaded, session)
                        }
                        Err(error) => ui.retry_missing_pattern_definition_recovery(
                            session,
                            format!(
                                "Matching recovery did not change the document: {error:#}"
                            ),
                        ),
                    }
                }
    ...
    fn install_recovered_missing_pattern_document(
        self: &Rc<Self>,
        loaded: toniator::persistence::LoadedDocument,
        session: Rc<MissingPatternDefinitionRecoverySession>,
    ) {
        if let Err(error) = toniator::render::decode_source(&loaded.document.source, 128) {
            self.retry_missing_pattern_definition_recovery(
                session,
                format!(
                    "Recovered candidate did not change the document because its artwork is invalid: {error:#}"
                ),
            );
            return;
        }
        self.gate_dirty_transition(move |ui| {
            ui.install_loaded_document(
                loaded.document.clone(),
                Some(session.source_path.clone()),
                true,
            );
            ui.show_message(
                "Pattern definition recovered — save to write the portable embedded authority.",
            );
        });
    }

    fn retry_missing_pattern_definition_recovery(
        self: &Rc<Self>,
        session: Rc<MissingPatternDefinitionRecoverySession>,
        feedback: impl Into<String>,
    ) {
        session.feedback.replace(Some(feedback.into()));
        glib::idle_add_local_once(glib::clone!(
            #[weak(rename_to = ui)]
            self,
            #[strong]
            session,
            move || ui.present_missing_pattern_definition_recovery(session)
        ));
    }
    ...
    fn change_selected_definition_channel_parameter(
        self: &Rc<Self>,
        channel: OutputChannelId,
        key: &str,
        value: LiteralValue,
        ...
        let document = {
            ...
            if !editor.set_selected_output_channel_parameter(channel, key, value) {
                drop(state);
                self.sync_controls();
                return;
            }
    ...
    fn sync_pattern_selector(
        ...
        let selected_preset = if shapes_selected {
            pattern_preset_index(PatternPresetAction::ApplyNamed(NamedPatternPreset::Grid))
                .expect("grid preset must be present in the preset inventory")
        } else if curves_selected {
            pattern_preset_index(PatternPresetAction::ActivateCurves)
                .expect("curves action must be present in the preset inventory")
        } else if weighted_voronoi_selected {
            pattern_preset_index(PatternPresetAction::ActivateWeightedVoronoi)
                .expect("Weighted Voronoi action must be present in the preset inventory")
            ...
            custom_name
                ...
                .and_then(|name| {
                    PATTERN_PRESET_ENTRIES
                        .iter()
                        .position(|entry| entry.label == name)
                })
                .map(|index| index as u32)
                .or_else(|| pattern_preset_index(PatternPresetAction::OpenCustomEditor))
                .expect("custom action must be present in the preset inventory")
        } else {
            pattern_preset_index(PatternPresetAction::ApplyNamed(NamedPatternPreset::Grid))
                .expect("grid preset must be present in the preset inventory")
        };
    ...
    fn sync_controls(&self) {
        ...
        if let Some(settings) = shape_settings.as_ref() {
            ...
            sync_web_scale(
                &self.channel_random_size_response,
                &self.channel_random_size_response_status,
    ...
    fn sync_channel_scope_panels(&self, pipeline: &ArtworkPipelineSettings, crosshatch: bool) {
        ...
        for controls in &self.channel_controls {
            ...
            sync_definition_channel_parameter_slots(
                controls,
                resolved_definition.as_ref(),
                controls.channel.belongs_to(output_model) && !crosshatch,
            );
        }
    ...
    fn update_actions(&self) {
        ...
        self.guided_definition_catalog.set_sensitive(has_document);
        self.custom_pattern_edit.set_sensitive(has_document);
        self.definition_library.set_sensitive(has_document);
        self.export
...
fn build_editor_view(
    ...
    let guided_definition_catalog = treatment_builder
        .object::<gtk::Button>("guided_definition_catalog")
        .expect("toniator-window.blp must define guided_definition_catalog");
    guided_definition_catalog.update_property(&[
        gtk::accessible::Property::Label("Guided Patterns"),
        gtk::accessible::Property::Description(
            "Browse immutable bundled definitions with schema-generated local controls",
        ),
    ]);
    ...
    let definition_library = treatment_builder
        .object::<gtk::Button>("definition_library")
        .expect("toniator-window.blp must define definition_library");
    definition_library.update_property(&[
        gtk::accessible::Property::Label("Pattern Library"),
        gtk::accessible::Property::Description(
            "Browse bundled, user-library, and project-embedded pattern definitions by stable ID",
        ),
    ]);
    ...
    sync_dropdown_strings(
        ...
        &["Sine", "Square Wave", "Sawtooth Wave"],
    );
    pattern_editor_curve_function
    ...
    let channel_random_size_response = builder_control_scale(
        &treatment_builder,
        BuilderScaleSpec {
            scale_id: "channel_random_size_response",
            control_id: "channel_random_size_response_control",
            label_id: "channel_random_size_response_label",
    ...
    channel_random_size_response.update_property(&[gtk::accessible::Property::Description(
        "Blend from uniform mark size at zero to source-responsive size at one",
    )]);
    let channel_random_size_response_status = treatment_builder
        .object::<gtk::Label>("channel_random_size_response_status")
        .expect("toniator-window.blp must define channel_random_size_response_status");
...
mod tests {
    ...
    #[test]
    fn channel_parameter_rows_follow_creator_schema_not_pattern_identity_or_labels() {
        let mut definition = toniator::load_bundled_wave_line_field_definition().unwrap();
        let instance = definition
            .default_instance_parameters(OutputChannelId::CMYK)
            .unwrap();
        definition.id = PatternId::new("custom.schema-driven-channel-controls.v1").unwrap();
        definition.display.name = "Unrelated display name".into();
        let resolved = toniator::ResolvedSelectedPatternDefinition {
            definition,
            instance,
        };
        assert_eq!(
            output_channel_numeric_parameters(&resolved)
                .into_iter()
                .map(|(parameter, _)| parameter.key.as_str())
                .collect::<Vec<_>>(),
            [
                "line-width-min",
                "line-width-max",
    ...
    #[test]
    fn missing_definition_recovery_uses_safe_default_destructive_loss_wording_and_stable_ids() {
        let candidate = toniator::MissingPatternDefinitionReplacementCandidate {
            id: PatternId::new("custom.recovery-label.v1").unwrap(),
            name: "Creator-facing recovery name".into(),
            provenance: PatternDefinitionSource::UserLibrary,
        };
        assert_eq!(MISSING_PATTERN_RECOVERY_CANCEL_RESPONSE, "Cancel");
        assert_eq!(
            MISSING_PATTERN_RECOVERY_LOCATE_RESPONSE,
            "Locate Matching .tnpattern"
        );
        assert_eq!(
            MISSING_PATTERN_RECOVERY_REPLACEMENT_RESPONSE,
            "Use Replacement (Discard Values)"
        );
        assert_eq!(
            missing_pattern_replacement_label(&candidate),
            "Creator-facing recovery name — user library — custom.recovery-label.v1"
        );
        assert_eq!(candidate.id.as_str(), "custom.recovery-label.v1");
    }

    #[test]
    fn missing_definition_recovery_feedback_keeps_requested_id_instruction_visible() {
        let detail = "Matching recovery did not change the document: selected definition has ID custom.other.v1, but custom.missing.v1 is required.";
        assert_eq!(
            missing_pattern_recovery_feedback_summary(detail),
            "No match: choose a current .tnpattern with the requested ID."
        );
        assert_eq!(
            missing_pattern_recovery_feedback_summary(
                "Replacement recovery did not change the document: no candidate was selected."
            ),
            "Replacement failed. Choose another definition or Cancel."
        );
        assert_eq!(
            missing_pattern_recovery_feedback_summary(
                "Recovered candidate did not change the document because its artwork is invalid."
            ),
            "Recovery could not load the artwork. Choose another matching definition."
        );
    }
    ...
    #[test]
    fn artifact_completion_quits_the_application_instead_of_only_closing_a_window() {
        assert_eq!(
            cli_artifact_completion(false),
            CliArtifactCompletion::QuitApplication,
            "a non-interactive artifact may have a modal or auxiliary window that outlives the main window"
        );
        assert_eq!(
            cli_artifact_completion(true),
            CliArtifactCompletion::KeepInteractive,
            "interactive runs retain normal window lifetime behavior"
        );
    }

    #[test]
    fn obsolete_curve_function_choices_are_rejected_instead_of_remapped() {
        assert_eq!(
            pattern_curve_function_from_choice("sawtooth").unwrap(),
            PatternCurveFunction::Sawtooth
        );
        let error = pattern_curve_function_from_choice("spiral").unwrap_err();
        assert!(error.contains("spiral"));
        assert!(error.contains("not supported"));
    }

    #[test]
    fn guided_catalog_prefers_the_current_editable_stable_selection() {
        let catalog = GuidedDefinitionCatalog {
            entries: vec![
                GuidedDefinitionCatalogEntry {
                    id: PatternId::COMPATIBILITY_SHAPES_V1,
                    name: "Unavailable".into(),
                    summary: "Unsupported metadata".into(),
                    editable: false,
                    unavailable_reason: Some("unsupported metadata".into()),
                },
                GuidedDefinitionCatalogEntry {
                    id: PatternId::COMPATIBILITY_CURVES_V1,
                    name: "First editable".into(),
                    summary: "Current schema".into(),
                    editable: true,
                    unavailable_reason: None,
                },
                GuidedDefinitionCatalogEntry {
                    id: PatternId::QUADRATIC_RADIAL_SPIRAL_V1,
                    name: "Second editable".into(),
                    summary: "Current schema".into(),
                    editable: true,
                    unavailable_reason: None,
                },
        ...
        assert_eq!(
            guided_catalog_initial_selection(
                &catalog,
        ...
        assert_eq!(
            guided_catalog_initial_selection(&catalog, Some(&PatternId::COMPATIBILITY_SHAPES_V1)),
            1,
            "an unavailable selected definition falls back to the first editable registry entry"
        );
        assert_eq!(guided_catalog_initial_selection(&catalog, None), 1);
    }

    #[test]
    fn graph_layout_accessibility_is_node_qualified_and_not_artwork_geometry() {
        assert_eq!(
            graph_layout_coordinate_accessibility("spiral", "X"),
            (
                "spiral graph layout X".into(),
                "X editor graph-layout coordinate for node `spiral`. This is not artwork geometry; changing it only changes the stored editor layout.".into(),
            )
        );
    }
    ...
    fn verify_realized_editor_controls_builder() {
        ...
        assert!(
            builder
                .object::<gtk::Scale>("channel_random_size_response")
                .is_some()
        );
        assert!(
            builder
                .object::<gtk::SpinButton>("channel_random_size_response_entry")
                .is_some()
        );
        for id in [
            "web_shape",
    ...
    #[test]
    fn pattern_preset_inventory_keeps_labels_and_actions_aligned() {
        assert_eq!(
            PATTERN_PRESET_ENTRIES,
            [
                PatternPresetEntry {
        ...
        assert_eq!(
            pattern_preset_entry(10).map(|entry| entry.action),
            Some(PatternPresetAction::ActivateCurves)
        );
        assert_eq!(
            pattern_preset_entry(11).map(|entry| entry.action),
            Some(PatternPresetAction::ActivateWeightedVoronoi)
        );
        assert_eq!(
            pattern_preset_entry(12).map(|entry| entry.action),
            Some(PatternPresetAction::OpenCustomEditor)
        );
        assert_eq!(pattern_preset_entry(13), None);
        assert_eq!(
            pattern_preset_index(PatternPresetAction::ActivateCurves),
            Some(10)
        );
        ...
        assert_eq!(
            pattern_preset_index(PatternPresetAction::OpenCustomEditor),
            Some(12)
        );
    }
    ...
    fn pattern_editor_extended_recipe_persists_and_renders_deterministically() {
        ...
        let creator_parameters = definition
            .parameters
            .iter()
            .filter_map(|parameter| match &parameter.authoring {
                ParameterAuthoring::Creator(_) => Some(parameter.key.as_str()),
                ParameterAuthoring::Internal => None,
            })
            .collect::<std::collections::BTreeSet<_>>();
        assert_eq!(listed, creator_parameters);
        let serialized_definition = toniator::serialize_tnpattern(&definition).unwrap();
        let reparsed_definition = toniator::parse_tnpattern(&serialized_definition).unwrap();
        assert_eq!(
            serialized_definition,
            toniator::serialize_tnpattern(&reparsed_definition).unwrap()
        );
        assert_eq!(
    ...
    #[test]
    fn pattern_editor_recipe_preserves_channel_treatment_and_structural_projection() {
        let source = SourceArtwork {
        ...
        settings.channels.r.random_size_response = 0.25;
        settings.channels.r.rotation = 17.0;
        settings.channels.r.grid_rotation = -13.0;
        settings.channels.g.point_sampler = WebShapePointSampler::Weighted;
        settings.channels.g.random_seed = 202;
        settings.channels.g.weight_influence = 3.5;
        ...
        assert_eq!(
            number_value(
                channel_instance_value(&instance, "channel.rgb.red", "random-size-response"),
                0.0
            ),
            0.25
        );
        assert_eq!(
            number_value(
                channel_instance_value(&instance, "channel.rgb.red", "rotation"),
                0.0
            ),
            17.0
        );
        assert_eq!(
            number_value(
                channel_instance_value(&instance, "channel.rgb.red", "grid-rotation"),
                0.0
            ),
            -13.0
        );
        assert_eq!(
            number_value(
                channel_instance_value(&instance, "channel.rgb.green", "channel-weight-influence"),
                0.0
            ),
            3.5
        );

        let mut structural_draft = draft.clone();
        structural_draft.placement = PatternPlacement::Random;
        structural_draft.density = 85.0;
        structural_draft.random_dispersion = PatternRandomDispersion::Poisson;
        let (_, structural_instance) = pattern_editor_recipe(&document, &structural_draft).unwrap();
        for (channel, key) in [
            ("channel.rgb.red", "channel-seed"),
            ("channel.rgb.red", "random-size-response"),
            ("channel.rgb.red", "rotation"),
            ("channel.rgb.green", "channel-weight-influence"),
        ] {
            assert_eq!(
                channel_instance_value(&structural_instance, channel, key),
                channel_instance_value(&instance, channel, key),
                "{key} must remain channel-instance treatment when the structural draft changes"
            );
        }
        assert_eq!(
            channel_instance_value(&structural_instance, "channel.rgb.red", "grid-rotation"),
            channel_instance_value(&instance, "channel.rgb.red", "grid-rotation"),
            "grid rotation is a retained structural compatibility projection that must not change during a draft edit"
        );
    }
