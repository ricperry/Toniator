...
use crate::pattern::{
    AffineTransform, ArtboardSpace, CanonicalBlendMode, CanonicalLayer, CanonicalPatternOutput,
    CanonicalPoint, FillRule as CanonicalFillRule, GeometryPolarity, NetworkPatternOutput,
    RegionPatternOutput,
};
...
pub fn generate_document_pattern_output_cancellable(
    ...
    token.checkpoint()?;
    // Resolved bundled and project-embedded definitions are semantic
    // authority. Both dispatch before the derived legacy RenderVariant facade,
    // which remains only for built-in compatibility renderers and transition
    // caches.
    let output = if let Some(resolved) =
        canonical.pattern_state.resolve_selected_bundled_pattern()?
    {
        crate::definition_runtime::execute_resolved_definition_with_source_cancellable(
            &resolved.definition,
            &resolved.instance,
    ...
    } else if let Some(embedded) = canonical.pattern_state.selected_embedded_pattern() {
        // Generic definitions (including document-local copies of bundled
        // Parametric Paths recipes) are authoritative. Existing Shapes custom
        // recipes still require their source-field executor, so retain that
        // capability as a fallback without selecting by pattern ID or name.
        match crate::definition_runtime::execute_resolved_definition_with_source_cancellable(
            &embedded.definition,
            &embedded.instance,
            &canonical.artwork_pipeline,
            ArtboardSpace {
...
mod tests {
    ...
    #[test]
    fn selected_bundled_spiral_uses_one_authoritative_canonical_output_for_preview_png_and_svg() {
        let mut editor = crate::model::DocumentEditor::new(Document::new(SourceArtwork {
            name: "spiral.png".into(),
            media_type: "image/png".into(),
            bytes: Arc::from(test_png()),
        }));
        assert!(editor.select_pattern(crate::pattern::PatternId::QUADRATIC_RADIAL_SPIRAL_V1));
        let document = editor.document();
        let output = generate_document_pattern_output(document).unwrap();
        let CanonicalPatternOutput::Paths(paths) = &output else {
            panic!("selected bundled Spiral must emit canonical paths");
        };
        assert_eq!(paths.geometry.width, 2);
        assert_eq!(paths.geometry.height, 2);
        assert_eq!(
            paths.geometry.layers.len(),
            crate::OutputChannelId::CMYK.len()
        );

        let expected_preview = render_canonical_pattern_output_cancellable(
            &output,
            2,
            2,
            true,
        ...
        .unwrap();
        assert_eq!(
            render_document_preview(document, 2, 17).unwrap().image,
            expected_preview
        );
        let png_options = crate::PngExportOptions {
            width: 2,
            height: 2,
            background: crate::PngBackground::White,
            channel: None,
        };
        let first_png = crate::png_bytes(document, png_options).unwrap();
        let second_png = crate::png_bytes(document, png_options).unwrap();
        assert_eq!(first_png, second_png);
        assert_eq!(
            image::load_from_memory(&first_png).unwrap().to_rgba8(),
            expected_preview
        );

        let expected_svg = crate::svg_export::curve_svg_bytes_cancellable(
            &paths.geometry,
            crate::svg_export::curve_svg_presentation_for_document(document),
            &CancellationToken::new(),
        )
        .unwrap();
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("spiral.svg");
        crate::export_svg(&path, document).unwrap();
        assert_eq!(std::fs::read(&path).unwrap(), expected_svg);
        usvg::Tree::from_data(&expected_svg, &usvg::Options::default()).unwrap();

        let mut contradictory = document.clone();
        ...
        cancelled.cancel();
        assert!(generate_document_pattern_output_cancellable(&contradictory, &cancelled).is_err());
    }

    #[test]
    fn document_local_spiral_copy_dispatches_through_its_complete_embedded_definition() {
        let mut definition = crate::load_bundled_quadratic_radial_spiral_definition().unwrap();
        definition.id = crate::pattern::PatternId::new("custom.render-spiral.1").unwrap();
        let instance = definition
            .default_instance_parameters(crate::OutputChannelId::CMYK)
            .unwrap();
        let mut editor = crate::model::DocumentEditor::new(Document::new(SourceArtwork {
            name: "custom-spiral.png".into(),
            media_type: "image/png".into(),
            bytes: Arc::from(test_png()),
        }));
        ...
        let CanonicalPatternOutput::Paths(paths) = &output else {
            panic!("document-local Parametric Paths recipe must emit canonical paths");
        };
        assert_eq!((paths.geometry.width, paths.geometry.height), (2, 2));
        assert_eq!(
            crate::png_export::document_artboard(editor.document()).unwrap(),
            (2, 2)
        );
