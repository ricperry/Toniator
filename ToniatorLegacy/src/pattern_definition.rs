...
use crate::{
    ...
    parametric_paths::ParametricPath,
    pattern::{
    ...
    structured_fields::StructuredFieldPaths,
...
/// Recipe v2 adds strict creator-parameter metadata. The outer JSON envelope
/// is unchanged, so format v1 remains current while v1 recipes are rejected.
pub const TNPATTERN_RECIPE_VERSION: u32 = 2;
...
pub struct PatternParameterDefinition {
    ...
    /// Explicit creator-facing metadata. This is required for all creator
    /// parameters; internal values must say so rather than relying on UI name
    /// inference or an omitted field.
    pub authoring: ParameterAuthoring,
}

#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(tag = "kind", rename_all = "kebab-case")]
#[serde(deny_unknown_fields)]
pub enum ParameterAuthoring {
    Creator(CreatorParameterMetadata),
    Internal,
}

#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct CreatorParameterMetadata {
    pub category: CreatorParameterCategory,
    pub unit: CreatorParameterUnit,
    pub increment: CreatorParameterIncrement,
    pub precision: u8,
    pub group: String,
    ...
    pub validation: ParameterValidationBehavior,
    pub serialization: ParameterSerializationBehavior,
    pub invalidation: ParameterInvalidationScope,
    /// Required only for a two-dimensional offset. This makes the relation
    /// machine-readable without inferring it from adjacent controls or names.
    pub two_dimensional: Option<TwoDimensionalRelation>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct TwoDimensionalRelation {
    pub pair_id: String,
    pub axis: TwoDimensionalAxis,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum TwoDimensionalAxis {
    X,
    Y,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum CreatorParameterCategory {
    BoundedNumber,
    IntegerCount,
    IntegerValue,
    Angle,
...
#[serde(tag = "kind", content = "value", rename_all = "kebab-case")]
pub enum CreatorParameterIncrement {
    None,
    Number(f64),
    Integer(u64),
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum ParameterValidationBehavior {
    Strict,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum ParameterSerializationBehavior {
    Always,
    OmitWhenDefault,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum ParameterInvalidationScope {
    Geometry,
    SourceSampling,
    Mapping,
    PresentationOnly,
...
pub enum PatternParameterConstraints {
    ...
    /// A strict numeric bound relative to another stable parameter. The
    /// relation is schema data, so Guided validation and persistence share it.
    NumberLessThanParameter {
        minimum: f64,
        maximum: f64,
        step: f64,
        parameter: String,
...
impl PatternParameterConstraints {
    fn value_type(&self) -> RecipeValueType {
        match self {
            Self::Number { .. } | Self::NumberLessThanParameter { .. } => RecipeValueType::Number,
...
pub enum RecipePortType {
    ...
    CurveModulatedPaths,
    /// A deterministic authored parametric centerline before any canonical
    /// geometry/emission stage consumes it.
    ParametricPath,
    /// A deterministic collection of continuous field paths before canonical
    /// geometry/emission consumes it.
...
const PARAMETRIC_PATHS_QUADRATIC_RADIAL_SPIRAL_PARAMETERS: [OperationParameterDescriptor; 11] = [
    OperationParameterDescriptor {
        name: "turns",
        value_type: RecipeValueType::Number,
    },
    ...
    OperationParameterDescriptor {
        name: "radial-growth-per-revolution",
        value_type: RecipeValueType::Number,
    },
    OperationParameterDescriptor {
        name: "spacing-growth-per-revolution",
        value_type: RecipeValueType::Number,
    },
    OperationParameterDescriptor {
        name: "starting-angle-degrees",
        value_type: RecipeValueType::Number,
    },
    OperationParameterDescriptor {
        name: "direction",
...
const PARAMETRIC_PATH_EMIT_INPUT: [OperationPortDescriptor; 1] = [OperationPortDescriptor {
    name: "path",
    kind: RecipePortType::ParametricPath,
}];
const STRUCTURED_FIELD_PATHS_WAVE_LINE_FIELD_PARAMETERS: [OperationParameterDescriptor; 6] = [
    OperationParameterDescriptor {
        name: "line-spacing",
        value_type: RecipeValueType::Number,
    },
    OperationParameterDescriptor {
        name: "orientation-degrees",
        value_type: RecipeValueType::Number,
    },
    OperationParameterDescriptor {
        name: "amplitude",
...
const STRUCTURED_FIELD_PATHS_EMIT_INPUT: [OperationPortDescriptor; 1] = [OperationPortDescriptor {
    name: "paths",
    kind: RecipePortType::StructuredFieldPaths,
}];
const PARAMETRIC_PATH_EMIT_PARAMETERS: [OperationParameterDescriptor; 3] = [
    OperationParameterDescriptor {
        name: "enabled",
        value_type: RecipeValueType::Boolean,
    },
...
const STRUCTURED_FIELD_EMIT_PARAMETERS: [OperationParameterDescriptor; 3] = [
    OperationParameterDescriptor {
        name: "enabled",
        value_type: RecipeValueType::Boolean,
    },
...
const STRUCTURED_FIELD_SOURCE_WIDTH_PARAMETERS: [OperationParameterDescriptor; 4] = [
    OperationParameterDescriptor {
        name: "line-width-min",
        value_type: RecipeValueType::Number,
    },
    ...
    OperationParameterDescriptor {
        name: "source-width-influence",
        value_type: RecipeValueType::Number,
    },
    OperationParameterDescriptor {
        name: "source-sampling-detail",
        value_type: RecipeValueType::Integer,
    },
];
...
pub static REGISTERED_OPERATIONS: OperationRegistry = OperationRegistry::new(&[
    ...
    RegisteredOperationDescriptor {
        id: "parametric-paths.quadratic-radial-spiral",
        version: 1,
        inputs: &NO_PORTS,
        output: OperationPortDescriptor {
            name: "path",
        ...
        parameters: &PARAMETRIC_PATHS_QUADRATIC_RADIAL_SPIRAL_PARAMETERS,
        canonical_output_kinds: &NO_CANONICAL_OUTPUT_KINDS,
    },
    RegisteredOperationDescriptor {
        id: "parametric-paths.emit-paths",
        version: 1,
        inputs: &PARAMETRIC_PATH_EMIT_INPUT,
        output: OperationPortDescriptor {
            name: "geometry",
            kind: RecipePortType::CanonicalGeometry,
        },
        parameters: &PARAMETRIC_PATH_EMIT_PARAMETERS,
        canonical_output_kinds: &PATH_OUTPUT_KIND,
    },
    RegisteredOperationDescriptor {
        id: "structured-fields.wave-line-field",
        version: 1,
        inputs: &NO_PORTS,
        output: OperationPortDescriptor {
            name: "paths",
        ...
        parameters: &STRUCTURED_FIELD_PATHS_WAVE_LINE_FIELD_PARAMETERS,
        canonical_output_kinds: &NO_CANONICAL_OUTPUT_KINDS,
    },
    RegisteredOperationDescriptor {
        id: "structured-fields.source-width",
        version: 1,
        inputs: &STRUCTURED_FIELD_PATHS_EMIT_INPUT,
        output: OperationPortDescriptor {
            name: "paths",
            kind: RecipePortType::StructuredFieldPaths,
        },
        parameters: &STRUCTURED_FIELD_SOURCE_WIDTH_PARAMETERS,
        canonical_output_kinds: &NO_CANONICAL_OUTPUT_KINDS,
    },
    RegisteredOperationDescriptor {
        id: "structured-fields.emit-paths",
        version: 1,
        inputs: &STRUCTURED_FIELD_PATHS_EMIT_INPUT,
        output: OperationPortDescriptor {
            name: "geometry",
        ...
        parameters: &STRUCTURED_FIELD_EMIT_PARAMETERS,
        canonical_output_kinds: &PATH_OUTPUT_KIND,
    },
    RegisteredOperationDescriptor {
...
pub enum RecipeRuntimeValue {
    ...
    StructuredFieldPaths(StructuredFieldPaths),
...
impl RecipeRuntimeValue {
    pub const fn port_type(&self) -> RecipePortType {
        match self {
            ...
            Self::ParametricPath(_) => RecipePortType::ParametricPath,
            Self::StructuredFieldPaths(_) => RecipePortType::StructuredFieldPaths,
...
impl PatternDefinition {
    ...
    fn validate_parameters(
        ...
        for parameter in &self.parameters {
            ...
            validate_parameter_authoring(parameter).map_err(|reason| {
                PatternDefinitionError::new(format!(
                    "pattern parameter `{}` creator metadata is invalid: {reason}",
                    parameter.key
                ))
            })?;
        ...
        validate_parameter_relations(&parameters)?;
        validate_cross_parameter_constraints(&parameters)?;
        validate_parameter_applicability(&parameters)?;
        ...
        for control in &self.quick_controls {
            ...
            if !is_safe_name(&control.id)
                ...
                || matches!(parameter.authoring, ParameterAuthoring::Internal)
            {
                return Err(PatternDefinitionError::new(
                    "quick controls must be unique, creator-facing, and match parameter scope",
                ));
            }
    ...
    pub fn validate_instance_parameters_with_registry(
        ...
        validate_instance_cross_parameter_constraints(
            &instance.pattern_values,
            DefinitionParameterScope::Pattern,
            &parameters,
        )?;
    ...
    fn resolve_instance_parameters<'a>(
        ...
        for parameter in self
            ...
            .filter(|parameter| parameter.scope == DefinitionParameterScope::Pattern)
        {
            if !parameter_requires_instance_value(parameter) {
                values
                    .entry(parameter.key.as_str())
    ...
    fn validate_layout(
        ...
        for section in &self.layout.sections {
            ...
            for parameter in &section.parameters {
                let Some(definition) = parameters.get(parameter.as_str()) else {
                    return Err(PatternDefinitionError::new(
                        "authoring layout has an invalid parameter reference",
                    ));
                };
                let ParameterAuthoring::Creator(metadata) = &definition.authoring else {
                    return Err(PatternDefinitionError::new(format!(
                        "authoring layout references internal parameter `{parameter}`"
                    )));
                };
                if metadata.group != section.id
                    || last_order.is_some_and(|order| metadata.display_order <= order)
                    || !listed.insert(parameter)
                {
                    return Err(PatternDefinitionError::new(
                        "authoring layout parameters must be creator-facing, grouped, unique, and ordered",
                    ));
                }
                last_order = Some(metadata.display_order);
            }
        }
        for parameter in parameters.values() {
            match &parameter.authoring {
                ParameterAuthoring::Creator(metadata) => {
                    // Structural Guided layout owns pattern parameters. Channel
                    // treatment metadata is consumed by Channel Settings and
                    // must not be forced into that separate draft surface.
                    if parameter.scope == DefinitionParameterScope::Pattern
                        && (!section_ids.contains(&metadata.group)
                            || !listed.contains(&parameter.key))
                    {
                        return Err(PatternDefinitionError::new(format!(
                            "creator parameter `{}` must appear exactly once in its declared authoring group `{}`",
                            parameter.key, metadata.group
                        )));
                    }
                }
                ParameterAuthoring::Internal if listed.contains(&parameter.key) => {
                    return Err(PatternDefinitionError::new(format!(
                        "internal parameter `{}` must not appear in authoring layout",
                        parameter.key
                    )));
                }
                ParameterAuthoring::Internal => {}
            }
        }
...
fn validate_parameter_authoring(
    parameter: &PatternParameterDefinition,
) -> Result<(), &'static str> {
    let ParameterAuthoring::Creator(metadata) = &parameter.authoring else {
        return Ok(());
    };
    if !is_safe_name(&metadata.group) || metadata.precision > 9 {
        return Err("group or displayed precision is invalid");
    }
    if !matches!(metadata.validation, ParameterValidationBehavior::Strict) {
        return Err("validation behavior is unsupported");
    }
    if metadata.required
        && !matches!(
            metadata.serialization,
            ParameterSerializationBehavior::Always
        )
    {
        return Err("required parameters must always serialize");
    }
    let valid_category = matches!(
        ...
        (
            ...
            CreatorParameterUnit::Unitless | CreatorParameterUnit::Pixels,
            CreatorParameterIncrement::Number(_)
        ) | (
            CreatorParameterCategory::IntegerCount,
            RecipeValueType::Integer,
            CreatorParameterUnit::Count,
            CreatorParameterIncrement::Integer(_)
        ) | (
    ...
    if !valid_category {
        return Err("category, value type, unit, or increment combination is unsupported");
    }
    let numeric_category = matches!(
        metadata.category,
        CreatorParameterCategory::BoundedNumber
            | CreatorParameterCategory::Angle
            | CreatorParameterCategory::Percentage
    ...
    if !numeric_category && metadata.precision != 0 {
        return Err("non-numeric creator parameters must use displayed precision 0");
    }
    if matches!(
        metadata.category,
    ...
    {
        return Err("integer creator parameters must use displayed precision 0");
    }
    if numeric_category {
        let CreatorParameterIncrement::Number(increment) = metadata.increment else {
            return Err("numeric creator parameters require a numeric increment");
        };
        if !matches!(metadata.category, CreatorParameterCategory::Percentage)
            && increment < 10f64.powi(-(metadata.precision as i32))
        {
            return Err("displayed precision cannot represent the numeric increment");
        }
    }
    if matches!(
        metadata.category,
        CreatorParameterCategory::Percentage | CreatorParameterCategory::NormalizedInfluence
    ) {
        let PatternParameterConstraints::Number {
            minimum, maximum, ..
        } = &parameter.constraints
        else {
            unreachable!("percentage already requires numeric constraints");
        };
        if *minimum < 0.0 || *maximum > 1.0 {
            return Err("percentage and normalized influence values must be between 0 and 1");
        }
    }
    if matches!(metadata.category, CreatorParameterCategory::Percentage) {
        let CreatorParameterIncrement::Number(increment) = metadata.increment else {
            unreachable!("percentage already requires a numeric increment");
        };
        if increment > 1.0 {
            return Err("percentage increment must be normalized between 0 and 1");
        }
        let Some(display_precision) = decimal_precision(increment * 100.0) else {
            return Err("percentage display increment is not representable at bounded precision");
        };
        if metadata.precision != display_precision {
            return Err("percentage precision must describe the displayed percent increment");
        }
    }
    if matches!(
        metadata.category,
        CreatorParameterCategory::TwoDimensionalOffset
    ) != metadata.two_dimensional.is_some()
    {
        return Err(
            "two-dimensional offset metadata must declare a pair and axis exactly when applicable",
        );
    }
    let increment_matches_constraints = match (&parameter.constraints, metadata.increment) {
        (
            PatternParameterConstraints::Number { step, .. }
            | PatternParameterConstraints::NumberLessThanParameter { step, .. },
            CreatorParameterIncrement::Number(value),
        ) => value.is_finite() && value > 0.0 && (*step - value).abs() <= 1e-12,
        (
            PatternParameterConstraints::Integer { step, .. },
            CreatorParameterIncrement::Integer(value),
        ) => *step == value && value > 0,
        (
            PatternParameterConstraints::Boolean
            | PatternParameterConstraints::Choice
            | PatternParameterConstraints::Text { .. }
            | PatternParameterConstraints::SvgAsset,
            CreatorParameterIncrement::None,
        ) => true,
        _ => false,
    };
    if !increment_matches_constraints {
        return Err("increment does not match the strict value constraints");
    }
    Ok(())
}

fn decimal_precision(value: f64) -> Option<u8> {
    (0..=9).find(|precision| {
        let scale = 10f64.powi(*precision as i32);
        ((value * scale).round() - value * scale).abs() <= 1e-9
    })
}

fn validate_parameter_relations(
    parameters: &HashMap<&str, &PatternParameterDefinition>,
) -> Result<(), PatternDefinitionError> {
    let mut pairs: HashMap<&str, Vec<(&PatternParameterDefinition, &CreatorParameterMetadata)>> =
        HashMap::new();
    for parameter in parameters.values() {
        let ParameterAuthoring::Creator(metadata) = &parameter.authoring else {
            continue;
        };
        if let Some(relation) = &metadata.two_dimensional {
            if !is_safe_name(&relation.pair_id) {
                return Err(PatternDefinitionError::new(format!(
                    "creator parameter `{}` has an invalid two-dimensional pair ID",
                    parameter.key
                )));
            }
            pairs
                .entry(relation.pair_id.as_str())
                .or_default()
                .push((parameter, metadata));
        }
    }
    for (pair_id, members) in pairs {
        if members.len() != 2 {
            return Err(PatternDefinitionError::new(format!(
                "two-dimensional pair `{pair_id}` must contain exactly one X and one Y parameter"
            )));
        }
        let (first_parameter, first_metadata) = members[0];
        let (second_parameter, second_metadata) = members[1];
        if first_metadata.category != CreatorParameterCategory::TwoDimensionalOffset
            || second_metadata.category != CreatorParameterCategory::TwoDimensionalOffset
            || first_metadata.group != second_metadata.group
            || first_metadata.ownership != second_metadata.ownership
            || first_parameter.scope != second_parameter.scope
            || first_metadata.unit != second_metadata.unit
            || first_metadata.axis() == second_metadata.axis()
        {
            return Err(PatternDefinitionError::new(format!(
                "two-dimensional pair `{pair_id}` must share category, group, ownership, scope, and unit with one X and one Y axis"
            )));
        }
    }
    Ok(())
...
fn validate_cross_parameter_constraints(
    ...
    for parameter in parameters.values() {
        let PatternParameterConstraints::NumberLessThanParameter {
            parameter: target,
            factor,
            ..
        } = &parameter.constraints
        ...
        let Some(target_parameter) = parameters.get(target.as_str()) else {
            return Err(PatternDefinitionError::new(format!(
                "pattern parameter `{}` references unknown relation parameter `{target}`",
                parameter.key
            )));
        };
        if !is_safe_name(target)
            || target == &parameter.key
            || parameter.scope != target_parameter.scope
            || parameter.value_type != RecipeValueType::Number
            || target_parameter.value_type != RecipeValueType::Number
            || !factor.is_finite()
            || *factor <= 0.0
        {
            return Err(PatternDefinitionError::new(format!(
                "pattern parameter `{}` has an invalid cross-parameter numeric relation",
                parameter.key
            )));
        }
        let LiteralValue::Number(value) = &parameter.default else {
            unreachable!("validated numeric default")
        };
        let LiteralValue::Number(target_value) = &target_parameter.default else {
            unreachable!("validated numeric target default")
        };
        if *value >= *factor * *target_value {
            return Err(PatternDefinitionError::new(format!(
                "pattern parameter `{}` default violates its cross-parameter relation",
                parameter.key
            )));
        }
    }
...
impl CreatorParameterMetadata {
    fn axis(&self) -> Option<TwoDimensionalAxis> {
        self.two_dimensional.as_ref().map(|relation| relation.axis)
    }
}

fn validate_parameter_applicability(
    parameters: &HashMap<&str, &PatternParameterDefinition>,
) -> Result<(), PatternDefinitionError> {
    let mut seen_display_slots = HashSet::new();
    let mut dependencies: HashMap<&str, Vec<&str>> = HashMap::new();
    for parameter in parameters.values() {
        let ParameterAuthoring::Creator(metadata) = &parameter.authoring else {
            continue;
        };
        if !seen_display_slots.insert((metadata.group.as_str(), metadata.display_order)) {
            return Err(PatternDefinitionError::new(format!(
                "creator display order {} is duplicated in group `{}`",
                metadata.display_order, metadata.group
            )));
        }
        if let ParameterApplicability::WhenParameterEquals {
            parameter: target,
            value,
        } = &metadata.applicability
        {
            let target_parameter = parameters.get(target.as_str()).ok_or_else(|| {
                PatternDefinitionError::new(format!(
                    "creator parameter `{}` applicability references unknown parameter `{target}`",
                    parameter.key
                ))
            })?;
            if parameter.scope == DefinitionParameterScope::Pattern
                && target_parameter.scope == DefinitionParameterScope::OutputChannel
            {
                return Err(PatternDefinitionError::new(format!(
                    "pattern creator parameter `{}` cannot depend on output-channel parameter `{target}`",
                    parameter.key
                )));
            }
            let ParameterAuthoring::Creator(_) = &target_parameter.authoring else {
                return Err(PatternDefinitionError::new(format!(
                    "creator parameter `{}` applicability references internal parameter `{target}`",
                    parameter.key
                )));
            };
            validate_parameter_value(target_parameter, value, &HashSet::new()).map_err(
                |reason| {
                    PatternDefinitionError::new(format!(
                        "creator parameter `{}` applicability value for `{target}` {reason}",
                        parameter.key
                    ))
                },
            )?;
            dependencies
                .entry(parameter.key.as_str())
                .or_default()
                .push(target_parameter.key.as_str());
        }
    }
    if graph_has_cycle(&dependencies, parameters.keys().copied()) {
        return Err(PatternDefinitionError::new(
            "creator parameter applicability conditions contain a cycle",
        ));
    }
    Ok(())
}
...
fn validate_instance_values(
    ...
    for parameter in parameters.values() {
        if parameter.scope == actual_scope
            && !values_by_key.contains(parameter.key.as_str())
            && parameter_requires_instance_value(parameter)
        {
            let location = channel
                .map(|channel| format!(" for output channel `{channel}`"))
                .unwrap_or_default();
...
fn validate_instance_cross_parameter_constraints(
    values: &[PatternInstanceValue],
        ...
        .filter(|parameter| parameter.scope == scope)
    {
        ...
        let Some(LiteralValue::Number(value)) = values.get(parameter.key.as_str()) else {
            continue;
        };
        let Some(LiteralValue::Number(target_value)) = values.get(target.as_str()) else {
            continue;
        };
        if *value >= *factor * *target_value {
            return Err(PatternInstanceParametersError::new(format!(
                "instance parameter `{}` must be less than {} times `{target}`",
...
fn parameter_requires_instance_value(parameter: &PatternParameterDefinition) -> bool {
    match &parameter.authoring {
        ParameterAuthoring::Creator(metadata) => {
            metadata.required
                || matches!(
        ...
        ParameterAuthoring::Internal => true,
    }
}
...
pub fn serialize_tnpattern_instance_parameters(
    ...
    canonical.pattern_values.retain(|value| {
        definition
            .parameters
            .iter()
            ...
            .is_some_and(|parameter| {
                !matches!(
                    ...
                    ParameterAuthoring::Creator(CreatorParameterMetadata {
                        required: false,
                        serialization: ParameterSerializationBehavior::OmitWhenDefault,
                        ..
                    })
...
mod tests {
    ...
    fn definition() -> PatternDefinition {
        ...
        for (order, parameter) in parameters.iter_mut().enumerate() {
            parameter.authoring = test_creator_metadata(
                parameter.value_type,
                &parameter.constraints,
                "weighted-voronoi",
                order as u32,
        ...
        PatternDefinition {
            format_version: 1,
            recipe_version: TNPATTERN_RECIPE_VERSION,
    ...
    fn parse_and_serialize_are_deterministic_and_strict() {
        ...
        let first = serialize_tnpattern(&current).unwrap();
        let parsed = parse_tnpattern(&first).unwrap();
        assert_eq!(first, serialize_tnpattern(&parsed).unwrap());
        let mut obsolete = definition();
        obsolete.recipe_version = 1;
        assert!(
            serialize_tnpattern(&obsolete)
                .unwrap_err()
                .to_string()
                .contains("recipe version")
        );
        assert!(
            parse_tnpattern(br#"{"format_version":1,"recipe_version":2,"unknown":true}"#).is_err()
        );
        assert!(parse_tnpattern(&[0xff]).is_err());
    }

    #[test]
    fn creator_parameter_contract_rejects_incomplete_or_conflicting_metadata() {
        let mut missing_authoring = serde_json::to_value(definition()).unwrap();
        missing_authoring["parameters"][0]
            .as_object_mut()
            .unwrap()
            .remove("authoring");
        assert!(parse_tnpattern(&serde_json::to_vec(&missing_authoring).unwrap()).is_err());
        assert!(
            serde_json::from_str::<ParameterApplicability>(
                r#"{"kind":"when-parameter-equals","parameter":"arrangement"}"#
            )
            .is_err()
        );
        assert!(
            serde_json::from_str::<ParameterApplicability>(r#"{"kind":"unsupported-condition"}"#)
                .is_err()
        );

        let mut duplicate = definition();
        duplicate.parameters[1].key = duplicate.parameters[0].key.clone();
        assert!(
            duplicate
                .validate()
                .unwrap_err()
        ...
        let mut invalid_default = definition();
        invalid_default.parameters[2].default = LiteralValue::Integer(1);
        assert!(
            invalid_default
                .validate()
                .unwrap_err()
        ...
        let mut invalid_increment = definition();
        let ParameterAuthoring::Creator(metadata) = &mut invalid_increment.parameters[2].authoring
        else {
            unreachable!()
        };
        metadata.increment = CreatorParameterIncrement::Integer(2);
        assert!(
            invalid_increment
                .validate()
                .unwrap_err()
        ...
        let mut invalid_precision = definition();
        let ParameterAuthoring::Creator(metadata) = &mut invalid_precision.parameters[0].authoring
        else {
            unreachable!()
        };
        ...
        let mut invalid_unit = definition();
        let ParameterAuthoring::Creator(metadata) = &mut invalid_unit.parameters[0].authoring
        else {
            unreachable!()
        };
        metadata.unit = CreatorParameterUnit::Count;
        assert!(
            invalid_unit
                .validate()
                .unwrap_err()
        ...
        let mut duplicate_order = definition();
        let first_order = match &duplicate_order.parameters[0].authoring {
            ParameterAuthoring::Creator(metadata) => metadata.display_order,
            ParameterAuthoring::Internal => unreachable!(),
        };
        let ParameterAuthoring::Creator(metadata) = &mut duplicate_order.parameters[1].authoring
        else {
            unreachable!()
        };
        metadata.display_order = first_order;
        assert!(
            duplicate_order
                .validate()
                .unwrap_err()
        ...
        let mut unknown_condition = definition();
        let ParameterAuthoring::Creator(metadata) = &mut unknown_condition.parameters[1].authoring
        else {
            unreachable!()
        };
        metadata.applicability = ParameterApplicability::WhenParameterEquals {
            parameter: "missing".into(),
            value: LiteralValue::Choice("shared".into()),
        };
        assert!(
            unknown_condition
                .validate()
        ...
        let ParameterAuthoring::Creator(left) = &mut cyclic.parameters[1].authoring else {
            unreachable!()
        };
        left.applicability = ParameterApplicability::WhenParameterEquals {
            parameter: "placement".into(),
            value: LiteralValue::Choice("source-weighted".into()),
        };
        let ParameterAuthoring::Creator(right) = &mut cyclic.parameters[5].authoring else {
            unreachable!()
        };
        right.applicability = ParameterApplicability::WhenParameterEquals {
            parameter: "arrangement".into(),
            value: LiteralValue::Choice("shared".into()),
        };
        assert!(cyclic.validate().unwrap_err().to_string().contains("cycle"));

        let mut cross_scope = definition();
        let mut pattern_parameter = parameter(
            "pattern-mode",
            DefinitionParameterScope::Pattern,
            RecipeValueType::Boolean,
            LiteralValue::Boolean(true),
        ...
        pattern_parameter.authoring = test_creator_metadata(
            RecipeValueType::Boolean,
            &pattern_parameter.constraints,
            "advanced",
            0,
            DefinitionParameterScope::Pattern,
        );
        let ParameterAuthoring::Creator(metadata) = &mut pattern_parameter.authoring else {
            unreachable!()
        };
        metadata.applicability = ParameterApplicability::WhenParameterEquals {
        ...
        cross_scope.parameters.push(pattern_parameter);
        cross_scope.layout.sections.push(AuthoringSection {
            id: "advanced".into(),
            label: "Advanced".into(),
            parameters: vec!["pattern-mode".into()],
        });
        assert!(
            cross_scope
                .validate()
                .unwrap_err()
                .to_string()
                .contains("cannot depend on output-channel")
        );

        let mut allowed_cross_ownership = definition();
        let ParameterAuthoring::Creator(placement) =
            &mut allowed_cross_ownership.parameters[5].authoring
        else {
            unreachable!()
        };
        placement.ownership = ParameterOwnership::PatternDefinition;
        let ParameterAuthoring::Creator(response) =
            &mut allowed_cross_ownership.parameters[7].authoring
        else {
            unreachable!()
        };
        response.applicability = ParameterApplicability::WhenParameterEquals {
            parameter: "placement".into(),
            value: LiteralValue::Choice("source-weighted".into()),
        };
        allowed_cross_ownership.validate().unwrap();

        let mut allowed_output_to_pattern = definition();
        allowed_output_to_pattern.parameters[1].scope = DefinitionParameterScope::Pattern;
        let ParameterAuthoring::Creator(arrangement) =
            &mut allowed_output_to_pattern.parameters[1].authoring
        else {
            unreachable!()
        };
        arrangement.ownership = ParameterOwnership::PatternDefinition;
        let ParameterAuthoring::Creator(response) =
            &mut allowed_output_to_pattern.parameters[7].authoring
        else {
            unreachable!()
        };
        response.applicability = ParameterApplicability::WhenParameterEquals {
        ...
        allowed_output_to_pattern.validate().unwrap();

        let mut omitted_creator = definition();
        omitted_creator.parameters[0].authoring = ParameterAuthoring::Internal;
        assert!(
            omitted_creator
                .validate()
                .unwrap_err()
        ...
        let mut missing_group = definition();
        let ParameterAuthoring::Creator(metadata) = &mut missing_group.parameters[0].authoring
        else {
            unreachable!()
        };
        metadata.group = "absent-group".into();
        assert!(
            missing_group
                .validate()
                .unwrap_err()
        ...
        let mut internal_layout = definition();
        internal_layout.parameters[0].authoring = ParameterAuthoring::Internal;
        assert!(
            internal_layout
                .validate()
                .unwrap_err()
    ...
    #[test]
    fn creator_numeric_categories_have_unambiguous_storage_and_display_rules() {
        let mut seed = definition();
        {
            let ParameterAuthoring::Creator(metadata) = &mut seed.parameters[3].authoring else {
                unreachable!()
            };
            metadata.category = CreatorParameterCategory::IntegerValue;
            metadata.unit = CreatorParameterUnit::None;
        }
        ...
        metadata.unit = CreatorParameterUnit::Count;
        assert!(seed.validate().unwrap_err().to_string().contains("unit"));

        for unit in [
            CreatorParameterUnit::Percent,
            CreatorParameterUnit::Normalized,
        ] {
            let mut bounded = definition();
            let ParameterAuthoring::Creator(metadata) = &mut bounded.parameters[4].authoring else {
                unreachable!()
            };
            metadata.category = CreatorParameterCategory::BoundedNumber;
            metadata.unit = unit;
            assert!(
                bounded
                    .validate()
        ...
        let mut percentage = definition();
        percentage.parameters[4].default = LiteralValue::Number(0.5);
        percentage.parameters[4].constraints = PatternParameterConstraints::Number {
            minimum: 0.0,
            maximum: 1.0,
            step: 0.001,
        };
        {
            let ParameterAuthoring::Creator(metadata) = &mut percentage.parameters[4].authoring
            else {
                unreachable!()
            };
            metadata.category = CreatorParameterCategory::Percentage;
            metadata.unit = CreatorParameterUnit::Percent;
            metadata.increment = CreatorParameterIncrement::Number(0.001);
            metadata.precision = 1;
        }
        percentage.validate().unwrap();
        {
            let ParameterAuthoring::Creator(metadata) = &mut percentage.parameters[4].authoring
            else {
                unreachable!()
        ...
        assert!(
            percentage
                ...
                .unwrap_err()
                .to_string()
                .contains("displayed percent increment")
        );
        {
            let ParameterAuthoring::Creator(metadata) = &mut percentage.parameters[4].authoring
            else {
        ...
        normalized.parameters[7].constraints = PatternParameterConstraints::Number {
            minimum: 0.0,
            maximum: 1.0,
            step: 0.001,
        };
        {
            let ParameterAuthoring::Creator(metadata) = &mut normalized.parameters[7].authoring
            else {
                unreachable!()
            };
            metadata.category = CreatorParameterCategory::NormalizedInfluence;
            metadata.unit = CreatorParameterUnit::Normalized;
        }
        ...
        let mut exponent = definition();
        {
            let ParameterAuthoring::Creator(metadata) = &mut exponent.parameters[7].authoring
            else {
                unreachable!()
            };
            metadata.category = CreatorParameterCategory::ResponseExponent;
            metadata.unit = CreatorParameterUnit::Unitless;
        }
        exponent.validate().unwrap();
        let serialized = serialize_tnpattern(&exponent).unwrap();
        assert_eq!(parse_tnpattern(&serialized).unwrap(), exponent);
    }

    #[test]
    fn two_dimensional_relations_are_explicit_and_complete() {
        let mut definition = definition();
        for (key, axis, order) in [
            ("offset-x", TwoDimensionalAxis::X, 0),
            ("offset-y", TwoDimensionalAxis::Y, 1),
        ] {
            ...
            let ParameterAuthoring::Creator(metadata) = &mut parameter.authoring else {
                unreachable!()
            };
            metadata.category = CreatorParameterCategory::TwoDimensionalOffset;
            metadata.unit = CreatorParameterUnit::DocumentRelativeDistance;
            metadata.two_dimensional = Some(TwoDimensionalRelation {
                pair_id: "offset".into(),
                axis,
            });
            definition.parameters.push(parameter);
        }
        definition.layout.sections.push(AuthoringSection {
            id: "offsets".into(),
            label: "Offsets".into(),
            parameters: vec!["offset-x".into(), "offset-y".into()],
        });
        definition.validate().unwrap();

        let mut duplicate_axis = definition.clone();
        let ParameterAuthoring::Creator(metadata) =
            &mut duplicate_axis.parameters.last_mut().unwrap().authoring
        else {
            unreachable!()
        };
        metadata.two_dimensional.as_mut().unwrap().axis = TwoDimensionalAxis::X;
        assert!(
            duplicate_axis
                .validate()
                .unwrap_err()
        ...
        let mut missing_axis = definition.clone();
        missing_axis.parameters.pop();
        missing_axis
            .layout
            .sections
            .last_mut()
        ...
        assert!(
            missing_axis
                ...
                .unwrap_err()
                .to_string()
                .contains("exactly one X and one Y")
        );

        let mut malformed = definition.clone();
        let ParameterAuthoring::Creator(metadata) =
            &mut malformed.parameters.last_mut().unwrap().authoring
        else {
            unreachable!()
        };
        metadata.two_dimensional.as_mut().unwrap().pair_id = "bad pair".into();
        assert!(
            malformed
                .validate()
                .unwrap_err()
    ...
    #[test]
    fn creator_parameter_optional_serialization_and_categories_are_deterministic() {
        let mut definition = definition();
        let ParameterAuthoring::Creator(metadata) = &mut definition.parameters[0].authoring else {
            unreachable!()
        };
        metadata.required = false;
        metadata.serialization = ParameterSerializationBehavior::OmitWhenDefault;
        let mut instance = definition
            .default_instance_parameters([OutputChannelId::CmykCyan])
            .unwrap();
        instance.output_channel_values[0]
            .values
            .retain(|value| value.key != "enabled");
        definition.validate_instance_parameters(&instance).unwrap();
        let serialized = serialize_tnpattern_instance_parameters(&definition, &instance).unwrap();
        let parsed = parse_tnpattern_instance_parameters(&definition, &serialized).unwrap();
        assert_eq!(
            serialized,
            serialize_tnpattern_instance_parameters(&definition, &parsed).unwrap()
        );

        let mut optional_always = definition.clone();
        let ParameterAuthoring::Creator(metadata) = &mut optional_always.parameters[1].authoring
        else {
            unreachable!()
        };
        metadata.required = false;
        metadata.serialization = ParameterSerializationBehavior::Always;
        let mut missing_optional_always = optional_always
            .default_instance_parameters([OutputChannelId::CmykCyan])
            .unwrap();
        missing_optional_always.output_channel_values[0]
            .values
            .retain(|value| value.key != "arrangement");
        assert!(
            optional_always
                .validate_instance_parameters(&missing_optional_always)
                .unwrap_err()
                .to_string()
                .contains("arrangement")
        );

        let mut optional_pattern = definition.clone();
        optional_pattern.parameters[1].scope = DefinitionParameterScope::Pattern;
        let ParameterAuthoring::Creator(metadata) = &mut optional_pattern.parameters[1].authoring
        else {
            unreachable!()
        };
        metadata.ownership = ParameterOwnership::PatternDefinition;
        metadata.required = false;
        metadata.serialization = ParameterSerializationBehavior::OmitWhenDefault;
        let mut omitted_pattern = optional_pattern
            .default_instance_parameters([OutputChannelId::CmykCyan])
            .unwrap();
        omitted_pattern
            .pattern_values
            .retain(|value| value.key != "arrangement");
        optional_pattern
            .validate_instance_parameters(&omitted_pattern)
            .unwrap();
        let resolved = optional_pattern
            .resolve_instance_parameters(&omitted_pattern, Some(OutputChannelId::CmykCyan))
            .unwrap();
        assert_eq!(
            resolved.get("arrangement"),
            Some(&&LiteralValue::Choice("shared".into()))
        );

        let mut optional_pattern_always = definition.clone();
        optional_pattern_always.parameters[1].scope = DefinitionParameterScope::Pattern;
        let ParameterAuthoring::Creator(metadata) =
            &mut optional_pattern_always.parameters[1].authoring
        else {
            unreachable!()
        };
        metadata.ownership = ParameterOwnership::PatternDefinition;
        metadata.required = false;
        metadata.serialization = ParameterSerializationBehavior::Always;
        let mut missing_pattern_always = optional_pattern_always
            .default_instance_parameters([OutputChannelId::CmykCyan])
            .unwrap();
        missing_pattern_always
            .pattern_values
            .retain(|value| value.key != "arrangement");
        assert!(
            optional_pattern_always
                .validate_instance_parameters(&missing_pattern_always)
                .unwrap_err()
                .to_string()
                .contains("arrangement")
        );

        let mut invalid_serialization = definition.clone();
        let ParameterAuthoring::Creator(metadata) =
            &mut invalid_serialization.parameters[1].authoring
        else {
            unreachable!()
        };
        ...
        assert!(
            invalid_serialization
                ...
                .unwrap_err()
                .to_string()
                .contains("always serialize")
        );

        let serialized_definition =
            String::from_utf8(serialize_tnpattern(&definition).unwrap()).unwrap();
        let invalid_invalidation = serialized_definition.replacen(
            "\"invalidation\":\"geometry\"",
            "\"invalidation\":\"unknown\"",
            1,
        );
        assert!(parse_tnpattern(invalid_invalidation.as_bytes()).is_err());

        let categories: HashSet<_> = [
            crate::bundled_pattern_definitions::load_bundled_shapes_definition().unwrap(),
            crate::bundled_pattern_definitions::load_bundled_curves_definition().unwrap(),
            crate::bundled_pattern_definitions::load_bundled_weighted_voronoi_definition().unwrap(),
        ]
        .into_iter()
        .flat_map(|definition| definition.parameters)
        .filter_map(|parameter| match parameter.authoring {
            ParameterAuthoring::Creator(metadata) => Some(metadata.category),
    ...
    fn parameter(
        ...
        PatternParameterDefinition {
            ...
            constraints,
            choices: vec![],
            authoring: ParameterAuthoring::Internal,
        }
    }
    ...
    fn test_creator_metadata(
        ...
        let (category, unit, increment) = match (value_type, constraints) {
            (RecipeValueType::Number, PatternParameterConstraints::Number { step, .. }) => (
                CreatorParameterCategory::BoundedNumber,
                CreatorParameterUnit::Unitless,
                CreatorParameterIncrement::Number(*step),
            ),
            (RecipeValueType::Integer, PatternParameterConstraints::Integer { step, .. }) => (
                CreatorParameterCategory::IntegerCount,
                CreatorParameterUnit::Count,
                CreatorParameterIncrement::Integer(*step),
            ),
            (RecipeValueType::Boolean, PatternParameterConstraints::Boolean) => (
                CreatorParameterCategory::Boolean,
                CreatorParameterUnit::None,
                CreatorParameterIncrement::None,
            ),
            (RecipeValueType::Choice, PatternParameterConstraints::Choice) => (
                CreatorParameterCategory::Enumeration,
                CreatorParameterUnit::None,
                CreatorParameterIncrement::None,
            ),
            (RecipeValueType::Text, PatternParameterConstraints::Text { .. }) => (
                CreatorParameterCategory::Text,
                CreatorParameterUnit::None,
                CreatorParameterIncrement::None,
            ),
            (RecipeValueType::SvgAsset, PatternParameterConstraints::SvgAsset) => (
                CreatorParameterCategory::SvgAsset,
                CreatorParameterUnit::None,
                CreatorParameterIncrement::None,
            ),
            _ => unreachable!("test parameter type and constraints agree"),
        };
        ParameterAuthoring::Creator(CreatorParameterMetadata {
            category,
            unit,
            increment,
            precision: if value_type == RecipeValueType::Number {
                3
            } else {
                0
            },
            group: group.into(),
            display_order,
            ownership: match scope {
                DefinitionParameterScope::Pattern => ParameterOwnership::PatternDefinition,
                DefinitionParameterScope::OutputChannel => ParameterOwnership::ChannelInstance,
            },
            required: true,
