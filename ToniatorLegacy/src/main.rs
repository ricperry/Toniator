...
pub struct CliOptions {
    ...
    artifact_guided_spiral: bool,
    artifact_guided_spiral_graph: bool,
    artifact_guided_definition: Option<String>,
    artifact_definition_library: bool,
    artifact_definition_library_project_copy: bool,
    artifact_definition_library_user_conflict: bool,
    artifact_definition_library_project_conflict: bool,
    artifact_missing_definition_recovery: bool,
}
...
impl CliOptions {
    pub fn artifact_mode(&self) -> bool {
        self.screenshot.is_some()
            ...
            || self.artifact_guided_spiral
            || self.artifact_guided_spiral_graph
            || self.artifact_guided_definition.is_some()
            || self.artifact_definition_library
            || self.artifact_definition_library_project_copy
            || self.artifact_definition_library_user_conflict
            || self.artifact_definition_library_project_conflict
            || self.artifact_missing_definition_recovery
    }
    ...
    fn parse() -> Self {
        ...
        while let Some(argument) = arguments.next() {
            match argument.to_str() {
                ...
                Some("--guided-spiral-editor") => options.artifact_guided_spiral = true,
                Some("--guided-spiral-graph") => options.artifact_guided_spiral_graph = true,
                Some("--guided-definition-editor") => {
                    options.artifact_guided_definition =
                        arguments.next().and_then(|value| value.into_string().ok())
                }
                Some("--definition-library") => options.artifact_definition_library = true,
                Some("--definition-library-project-copy") => {
                    options.artifact_definition_library_project_copy = true
                }
                Some("--definition-library-user-conflict") => {
                    options.artifact_definition_library_user_conflict = true
                }
                Some("--definition-library-project-conflict") => {
                    options.artifact_definition_library_project_conflict = true
                }
                Some("--missing-definition-recovery") => {
                    options.artifact_missing_definition_recovery = true
                }
                Some("--screenshot") => {
                ...
                Some("--help") | Some("-h") => {
                    println!(
                        "Toniator native GTK application\n\n  --demo                 Open built-in artwork\n  --guided-spiral-editor Artifact: open the shared local Spiral editor\n  --guided-spiral-graph  Artifact: open that editor's Graph view\n  --definition-library   Artifact: open the native pattern library\n  --definition-library-project-copy\n                         Artifact: show the library with an embedded project copy\n  --definition-library-user-conflict\n                         Artifact: show destructive user-file conflict wording\n  --definition-library-project-conflict\n                         Artifact: show project-protected import wording\n  --missing-definition-recovery\n                         Artifact: show missing stable-ID recovery wording\n  --preview-surface V    Artifact preview: checkerboard or #RRGGBBAA\n  --export-background V  Artifact export: none or #RRGGBBAA\n  --expand-document      Expand Document controls for screenshots\n  --screenshot PATH      Save the actual application window as PNG\n  --export-svg PATH      Export the demo as editable SVG\n  --export-png PATH      Export the demo as a PNG image"
                    );
                    std::process::exit(0);
                }
                _ => {}
...
mod tests {
    ...
    fn save_and_export_load_demo_content_but_screenshot_only_stays_quiet() {
        ...
        let library = CliOptions {
            artifact_definition_library: true,
            ..Default::default()
        };
        assert!(library.loads_example());
        assert!(library.artifact_mode());

        let project_copy = CliOptions {
            artifact_definition_library_project_copy: true,
            ..Default::default()
        };
        assert!(project_copy.loads_example());
        assert!(project_copy.artifact_mode());

        let conflict = CliOptions {
            artifact_definition_library_user_conflict: true,
            ..Default::default()
        };
        assert!(conflict.loads_example());
        assert!(conflict.artifact_mode());

        let missing_definition = CliOptions {
            artifact_missing_definition_recovery: true,
            ..Default::default()
        };
        assert!(missing_definition.loads_example());
        assert!(missing_definition.artifact_mode());
    }
