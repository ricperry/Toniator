...
definitions, a Shapes-specific pattern editor, and strict document v10 / preset
...
integration advanced strictly to document v10 / `.tntr` v6 and rejects obsolete
or incomplete definitions without migration or defaulting. Evidence is cached
...
**Status:** Incomplete preservation checkpoint; corrective Gates 1–5 and Gate
6A, 6B1, 6B2A, and 6B2B are parent accepted. Gate 7 cross-family proof is
next and TON-010 remains **Open**.
...
Embedded custom Shapes definitions are persisted in document v10 / `.tntr` v6
...
Shapes-compatible graph. Its typed 13-entry preset inventory now keeps labels,
actions, selected-state lookup, and bounds checks aligned; the remaining
`apply_named_pattern_preset` and `pattern_editor_recipe` paths still encode
legacy per-pattern defaults, fixed node IDs, operation IDs, and parameter keys.
The native `shapes.lattice-placement-editor` operation then branches internally
among grid, triangular, curve, math, and random variants. Adding another
creative option still risks another UI match arm and native special case
instead of composing registered operations through a stable, user-inspectable
contract. Gate 3C removed the named Spiral preset mutation/default route; the
accepted Gate 4 Guided editor also removes the Math Function `Spiral` dropdown
and its Shapes preview/recipe branch.

The intended ownership boundary is:

* **Pattern definition:** reusable construction/topology, grid or path
  construction, structural spacing/orientation, curve or math family,
  randomness/distribution policy, jitter algorithm, point/connected-path
  output, and other reusable construction choices. Definitions declare
  supported emitted geometry but do not select per-ink marks or mark scale.
* **Channel instance:** enabled/color/opacity, sampling and coverage-density
  response, source weighting/influence, random seed, selected mark primitive,
  mark scale, mark or treatment rotation, and selection visibility. Per-channel
  controls belong only in the main window.
...
**Status:** Complete — parent accepted. Gates 2–5 and Gate 6A are also parent
accepted; Gate 6B1, Gate 6B2A, and Gate 6B2B are also parent accepted. Gate 7
is next and TON-010 remains Open.

`docs/TON-010_PATTERN_CONTROL_OWNERSHIP.md` now records every current
pattern-related control, its current storage authority, intended owner,
classification, compatibility projection, and disposition. The contract keeps
`Document.pattern_state` semantic: definitions own reusable construction,
topology, randomness policy, and structural orientation; channel instances own
enabled/color/opacity/sampling/coverage-density response/source weighting/seed/
mark primitive/mark scale/mark or treatment rotation/selection visibility. The
proven Shape Size Response leak was
renamed from `pattern_editor_random_size_response` to
`channel_random_size_response` and removed from `PatternEditorDraft`; focused
recipe coverage proves structural-draft changes preserve channel treatment
values and retained structural grid-rotation projection. This gate does not
move current compatibility storage, alter canonical output, begin a parameter
schema, or change Weighted Voronoi placement/tessellation.

Additional verified gaps:

* **Gate 6A resolver/model foundation is parent accepted.**
  `UserPatternLibrarySnapshot` discovers direct user `.tnpattern` files under
  the shared XDG policy in deterministic path order and retains malformed-file
  diagnostics. `PatternDefinitionLifecycleResolver` supplies bundled/user/
  project layering, immutable bundled-ID conflicts, project-over-user
  diagnostics, atomic reload inputs, and missing-ID recovery candidates without
  changing `Document.pattern_state` or render/export authority. A failed strict
  conflict reload preserves the prior snapshot, selection, resolved
  definition/fingerprint/source, and diagnostics. The full locked matrix passed
  301 library and 61 binary/UI tests; no UI artifact applies.
* **Gate 6B1 native lifecycle workflow is parent accepted.** Pattern Library
  presents bundled, user-library, and project definitions by stable ID with
  provenance, malformed-file diagnostics, and explicit Reload. Its user-only
  action embeds the complete definition with validated defaults for active
  output channels as one undoable
  `Document.pattern_state` edit, so save/reopen no longer depends on the
  external library. The shared local draft can atomically Save Draft to Library
  with full graph/schema/layout/assets payload, current definition-owned
  structural values promoted to `.tnpattern` defaults, and discovery
  verification while retaining draft/project selection on failure; per-channel
  instance values stay project-local. The locked matrix passed 305 library and 61 binary/UI
  tests; the realized isolated-XDG Wayland library artifact is implementation
  evidence only. The isolated capture SHA-256 is
  `74f31c07df6e80dced0b2780bea3b161b5c8238ac5a614b20241cff8f8a34dc9`.
* **Gate 6B2A external import and project-copy editing is parent accepted.**
  Import strictly parses a current-schema
  external `.tnpattern` before any mutation, explicitly deduplicates identical
  content, offers cancel/destructive atomic user-file replace/new-ID only when
  one valid matching user file exists, and surfaces duplicate paths with
  cancel/new-ID only. Bundled authority is immutable; project conflict directs
  creators to Edit Project Copy or new-ID. The native
  library now exposes generic Edit Project Copy, reopening the complete stored
  definition and current instance in the shared Guided/Graph draft; Apply is
  one edit, Cancel is inert, and undo/redo/reopen remain exact. The locked
  matrix passed 309 library and 61 binary/UI tests; four isolated-XDG Wayland
  captures are implementation evidence only. Their hashes are recorded in the
  parent-acceptance evidence and architecture map.
* **Gate 6B2B missing-definition recovery is parent accepted.** Current-schema
  document-open diagnosis now retains only a typed missing-selected-stable-ID
  candidate after strict decode and unrelated validation; the invalid document
  never reaches the live editor. Native recovery offers Cancel, strict exact
  `.tnpattern` lookup with preserved-instance validation, or an explicit
  stable-ID replacement with fresh defaults and no structural-value mapping.
  Recovered custom authority embeds portably and installs dirty. There is no
  migration, defaulting fallback, or automatic replacement. The locked matrix
  passed 314 library and 63 binary/UI tests. A transient parent D-Bus `NoReply`
  was followed by the exact isolated test and two later full passes without a
  code workaround. The final isolated-XDG artifact hash is
  `5ce73287a14dd726ae33d255c1db4321f7dad177911a05fb29acea432f87def4` and is
  implementation evidence only. Human acceptance remains open.
...
1. **Complete:** freeze and test the pattern-definition versus channel-instance
   ownership table; remove or rename proven leaky draft/widget state without
   changing output.
2. **Complete — Gate 2:** establish the strict typed parameter-schema
   foundation recorded in `docs/TON-010_PATTERN_PARAMETER_SCHEMA.md`. Recipe
   v1 is rejected without migration/defaulting; no GTK control or recipe
   semantic was added.
3. **Complete — Gate 3 parent accepted:**
   `parametric-paths.quadratic-radial-spiral.v1` is selected by stable ID from
   the required `Document.pattern_state.bundled_definition_instances` map and
   resolved through one generic canonical-path executor before compatibility
   dispatch. Deterministic preview, PNG, and editable SVG parity is covered by
   automated canonical artifacts. The named Spiral preset mutation/default
   injection is removed. Removing the entry exposed a preset-index regression;
   one typed 13-entry inventory now keeps labels and actions aligned and has
   focused bounds/mapping coverage. No selector UI, GTK control,
   family/display/index branch, Shapes lattice route, or migration/defaulting
   path was added. The current executor is intentionally Paths-only/provider-
   limited; broader canonical output algebra remains open.
4. **Complete — Gate 4 parent accepted — Spiral Guided editor:** the
   registry-backed stable-ID catalog follows generic selection policy (current
   editable stable document selection, otherwise the first editable registry
   entry; the artifact fixture explicitly requests Spiral). It generates an
   immutable local Guided draft only from strict definition-owned schema
   metadata, supplying units, contextual help, validation feedback, declared
   conditional enablement, metadata focus order, accessible names/descriptions,
   and local canonical preview updates. Channel-instance values remain in
   Channel Settings. Bundled definitions remain immutable and require explicit
   duplication to a document-local copy before Apply. The superseded
   Shapes-compatible Spiral authoring choice/branch is removed; obsolete values
   are rejected rather than remapped or defaulted. The corrected non-interactive
   artifact path generically quits the application after capture. The full
   locked suite passed 285 library and 60 binary/UI tests; the settled GTK
   artifact is implementation evidence only. Gate 5 is separate work.
5. **Complete — Gate 5 parent accepted — shared Guided/Graph draft:** Guided
   and the actual bounded Graph view share one complete non-lossy current-schema
   draft. Graph edits stored layout coordinates only; it is not a topology,
   argument, asset, or schema editor. Apply is one atomic document edit, Cancel
   is non-mutating, rejected Apply preserves the exact draft for corrected-ID
   retry, and undo/redo restore the complete relationship. Draft/model
   boundaries reject built-in and immutable bundled-ID collisions; accessibility
   relations, local identity, retry, and current-schema unexposed-content
   preservation are covered. The full locked suite passed 292 library and 61
   binary/UI tests; refreshed Wayland artifacts are implementation evidence
   only. Gate 6A and Gate 6B1 are parent accepted.
6. **Complete — Gate 6 corrective lifecycle:** Gate 6B2A strict external
   import/project-copy editing and Gate 6B2B current-schema missing-definition
   diagnosis/recovery are parent accepted. The final matrix passed 314 library
   and 63 binary/UI tests; human acceptance remains open.
7. **Gate 7 — sequential cross-family proof:** deliver one Structured Fields
   proof and one Stochastic Distributions proof through the same public schema,
   shared draft, Guided editor, lifecycle, and canonical runtime. Do not add
   family-specific GTK branches or alter protected Weighted Voronoi geometry.
8. **Gate 8 — remove superseded authority:** remove compatibility dispatch and
   duplicate parameter authority only after canonical output-equivalence tests
   pass.
9. **Gate 9 — full validation and human handoff:** complete automated parity
   plus the outstanding human Stage 5 and creative workflow acceptance gates.
   Only then reconcile TON-010 as Done.

#### Corrective Gate 2 — parameter-schema contract, 2026-08-02

**Status:** Complete; TON-010 remains Open.

`docs/TON-010_PATTERN_PARAMETER_SCHEMA.md` is the exact current contract for
creator/internal parameter metadata. It distinguishes `IntegerValue` seeds
with unit `None` from `IntegerCount`, normalized stored percentages from their
percent display/increment/precision rule, `[0, 1]` normalized influence from
unitless response exponents that may exceed one, and canonical
document/artboard distance from pixels. Current bundled definitions were
reclassified only where their operation semantics make that answer clear, then
validated and deterministically round-tripped as schema artifacts.

Current `.tnpattern` format v1 / recipe v2 validation rejects recipe v1. There
is no migration or defaulting path. This closes the parameter contract only:
no GTK control, generic Guided/Graph consumer, recipe operation, canonical
output, or visual rendering changed. Schema-generated Spiral Guided controls
remain Gate 4; the shared Guided/Graph draft remains Gate 5. The focused and
full automated evidence is recorded under
`.codex-work/evidence/`; accepted Gate 3C adds selected-Spiral PNG/SVG
artifacts but does not claim visual acceptance.

The reusable audit is cached at
`.codex-work/evidence/ton-010-preservation-checkpoint-audit-2026-08-02.md`.

#### Corrective Gate 3 — immutable Spiral and generic resolved execution, 2026-08-02

**Status:** Complete — parent accepted; Gates 4 and 5 are also parent accepted
and Gate 6A is parent accepted. Gate 6B1 is also parent accepted; Gate 6B2 is
next. TON-010 remains **Open**.

The immutable bundled definition `parametric-paths.quadratic-radial-spiral.v1`
is selected by its stable `PatternId` through strict Document v10
`pattern_state.bundled_definition_instances`. The generic resolved-definition
executor runs the registered graph for each declared semantic channel and
combines canonical Paths without inspecting a bundle label, display name,
selector index, Spiral ID, or parametric family. Preview, PNG, and editable SVG
consume that same canonical output. The route is intentionally a Paths-only
provider proof at this gate; other canonical output kinds remain a later
bounded output-algebra stage, and no selector UI was added.

Parent review corrected a preset-index regression left by removing the named
Spiral entry. A typed 13-entry preset inventory now owns labels, actions,
selected-state lookup, and bounds checks. The named Spiral preset
mutation/default injection is removed. The accepted Gate 4 Guided editor
also removes the Shapes-compatible Pattern Editor Math Function `Spiral`
dropdown, fixed choice value, and preview/recipe branch; the immutable
Parametric Paths definition is its sole current authoring route. Evidence and
artifacts are recorded in
`.codex-work/evidence/ton-010-gate-3c-generic-selected-spiral-parity-2026-08-02.md`;
human GNOME/Wayland, Krita/Inkscape, and creative acceptance remain open.

#### Corrective Gate 4 — schema-generated Guided Spiral editor, 2026-08-02

**Status:** Complete — parent accepted. Gate 5 is also parent accepted; Gate 6
is next and TON-010 remains **Open**.

`GuidedDefinitionCatalog` lists current bundled definitions by stable ID and
uses generic default policy: the current editable stable document selection,
otherwise the first editable registry entry; only the artifact fixture
explicitly requests Spiral. Unsupported metadata remains visible with an exact
reason. `GuidedDefinitionDraft` uses layout and strict Creator metadata rather
than parameter names, fixed Spiral GTK controls, numeric selector positions, or
graph node IDs. It accepts only Pattern Definition-owned values and deliberately
excludes channel `enabled`, `color`, and `opacity`. Each local change validates
the instance and refreshes the same generic canonical executor/renderer route
used by the Spiral runtime. The dialog explicitly says bundled definitions are
immutable and require explicit duplication to a document-local copy before any
Gate 5 Apply.

The focused test suite proves every exposed structural value affects the local
canonical preview and covers edge extension. The GTK artifact shows the real
dialog, visible local canonical preview, metadata help/units, and no fake
Graph or Apply control. Artifact capture now exits generically through
application quit even when a modal remains open. The settled artifact and the
285-library/60-binary automated matrix are implementation evidence only, not
human creative, accessibility, or external-application acceptance. The old
visible Shapes Spiral choice and its obsolete values are removed/rejected; the
remaining Math Function generators are kept only while their current strict
definitions are supported.

#### Corrective Gate 5 — shared non-lossy Guided/Graph draft, 2026-08-02

**Status:** Complete — parent accepted. Gate 6A, Gate 6B1, and Gate 6B2A are
parent accepted; Gate 6B2B is also parent accepted. Gate 7 is next and TON-010
remains **Open**.

`SharedRecipeEditorDraft` is one complete strict-schema draft shared by Guided
stable-ID bindings and the actual bounded Graph view. Graph lists stored
nodes/edges and edits layout X/Y only; it is deliberately not a topology,
operation-argument, asset, or schema editor. Valid current-schema content not
exposed by the bounded UI is preserved through Guided edits, Graph layout edits,
explicit duplication, and JSON round trips.
...
built-in and immutable bundled IDs. Guided and node-qualified Graph controls
have accessible labels/relations, and the local ID/provisional inherited name
remain visible. Strict current-schema validation continues to reject unknown
operation IDs and fields.

The locked suite passed 292 library and 61 binary/UI tests. Refreshed Wayland
Guided/Graph captures are implementation evidence only. Gate 6A, Gate 6B1, and
Gate 6B2A and Gate 6B2B are now parent accepted. Gate 7 is the exact next
substage: sequentially prove one Structured Fields and one Stochastic
Distributions pattern through the same public schema/editor/runtime. Human
GNOME/Wayland, keyboard, screen-reader, creative, Krita, and Inkscape
acceptance remains open.

#### Corrective Gate 6A — definition-lifecycle resolver/model foundation, 2026-08-02

**Status:** Complete — parent accepted. Gate 6B1 and Gate 6B2A are also parent
accepted; Gate 6B2B is also parent accepted; Gate 7 is next and TON-010 remains
**Open**.

`PatternDefinitionLifecycleResolver` provides one presentation-neutral layered
resolution surface around the existing registry for immutable bundled,
deterministic XDG user-library, and project-embedded definitions. Direct
`.tnpattern` files are discovered in stable path order; malformed files remain
non-fatal diagnostics. Layer precedence, immutable bundled-ID conflicts,
project-over-user diagnostics, and typed missing-ID recovery candidates are
explicit. Reload constructs a complete replacement before assignment: a failed
strict-conflict reload preserves the previous user snapshot, selected identity,
resolved definition/fingerprint/source, and diagnostics. Save As shares the XDG
path policy; Gate 6B1 now provides the native Save Draft consumer, Gate 6B2A
accepts external import/open, and missing-definition recovery remains Gate
6B2B.

The locked Gate 6A suite passed 301 library and 61 binary/UI tests, with
formatting, strict Clippy, locked release, 0 doctests, diff, and protected-path
checks passing. No GTK artifact applies because Gate 6A adds no UI behavior.
Gate 6B1 and Gate 6B2A are accepted native lifecycle consumers, and Gate 6B2B
is the accepted current-schema missing-definition recovery boundary. Human
acceptance remains open.

#### Corrective Gate 6B2A — strict external import and project-copy editing, 2026-08-02

**Status:** Complete — parent accepted. Gate 6B2B is also parent accepted;
Gate 7 is next and TON-010 remains **Open**.

External `.tnpattern` input is strictly parsed before mutation and classified by
stable ID as ready, identical, user-library conflict, immutable bundled
conflict, or project-protected conflict. Identical content is a no-write
dedupe. Destructive replacement is atomic and available only for exactly one
matching user file; duplicate valid user files permit only Cancel or import
under a new custom ID. Bundled and project authority cannot be overwritten.
New-ID imports preserve graph, schema, layout, assets, metadata, and definition
defaults exactly. Project-embedded rows expose generic Edit Project Copy; the
complete definition and current instance reopen in the shared Guided/Graph
draft, and Apply, undo/redo, save, and reopen preserve structural, layout, and
channel values.

The locked Gate 6B2A suite passed 309 library and 61 binary/UI tests. Four
isolated-XDG Wayland captures are implementation evidence only; their hashes
are recorded in the parent-acceptance evidence and architecture map. Gate 6B2B
is documented below as parent accepted. Human acceptance remains open.

#### Corrective Gate 6B2B — current-schema missing-definition recovery, 2026-08-02

**Status:** Complete — parent accepted. Gate 7 is next and TON-010 remains
**Open**.

Current-format document open yields either a valid document or a typed
missing-selected-definition candidate only when the selected stable ID is the
sole missing authority and all unrelated validation succeeds. Exact recovery
accepts only a strict current `.tnpattern` with the requested stable ID,
validates retained values, embeds portable authority, and canonicalizes the
complete document before installation. Explicit replacement uses stable-ID
resolution and fresh defaults; it never maps incompatible structural values.
Resolver conflicts disable replacement without disabling exact-file recovery.
The native recovery session is Cancel-default, retains candidates through
chooser cancellation/failure, exposes creator name/provenance/stable ID and
accessible diagnostics, and does not alter the source or editor until complete
recovery succeeds.

The final matrix passed 314 library and 63 binary/UI tests. A transient parent
D-Bus `NoReply` was followed by the exact isolated test and two later full
passes without a code workaround. The final isolated-XDG artifact hash is
`5ce73287a14dd726ae33d255c1db4321f7dad177911a05fb29acea432f87def4`; it is
implementation evidence only. Human chooser, keyboard, screen-reader,
save/reopen, and creative acceptance remain open.
...
rejection review. Bundled and embedded recipe loading and strict document v10 /
`.tntr` v6 persistence have landed. Corrective Gate 6 is parent accepted;
proceed to Gate 7's sequential Structured Fields and Stochastic Distributions
proofs, followed by the broader local library, import/export, layered
resolution, and project-embedding/recovery review. Continue rejecting obsolete definitions;
