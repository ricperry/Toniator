...
mod tests {
    ...
    #[test]
    fn preset_value_mode_is_canonical_and_removed_inverse_is_rejected() {
        assert_eq!(parse_value_mode("luminance").unwrap(), ValueMode::Luminance);
        assert!(parse_value_mode("inverted-luminance").is_err());
        assert_eq!(value_mode_name(ValueMode::Luminance), "luminance");
        ...
        .unwrap();
        let text = std::str::from_utf8(&bytes).unwrap();
        assert!(text.contains("\"valueMode\": \"luminance\""));
        assert!(!text.contains("inverted-luminance"));
    }
}
