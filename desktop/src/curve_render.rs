...
pub fn generate_curve_geometry(
    ...
    for ink in Ink::ALL {
        ...
        let (r, g, b) = parse_hex_color(&channel.color)
...
pub fn render_curve_geometry(
    ...
    let scale = (max_dimension as f32 / geometry.width.max(geometry.height) as f32).min(1.0);
...
mod tests {
    ...
    fn synthetic_reference_geometry_is_stable() {
        ...
        // Generated by the authoritative web `generateHalftoneSvg` fallback
        // sampler for the same synthetic settings. Browser path-length
        // implementations are allowed their documented sub-pixel tolerance.
        const WEB_FIRST: &str = "M 74.251 -17.14 C 77.294 -14.615 79.437 -9.877 83.379 -9.566 C 88.672 -9.147 93.301 -13.349 98.261 -15.241 C 96.625 -19.164 94.989 -23.086 93.353 -27.009 C 90.028 -25.444 86.976 -23.062 83.379 -22.316 C 82.428 -22.118 83.929 -24.178 84.204 -25.11 C 80.886 -22.453 77.569 -19.797 74.251 -17.14 Z";
        const WEB_LAST: &str = "M 26.647 107.009 C 29.972 105.444 33.024 103.062 36.621 102.316 C 37.572 102.118 36.071 104.178 35.796 105.11 C 39.114 102.453 42.431 99.797 45.749 97.14 C 42.706 94.615 40.563 89.877 36.621 89.566 C 31.328 89.147 26.699 93.349 21.739 95.241 C 23.375 99.164 25.011 103.086 26.647 107.009 Z";
        assert_path_close(&paths[0], WEB_FIRST, 0.08);
        assert_path_close(&paths[paths.len() - 1], WEB_LAST, 0.08);
    }
    ...
    fn assert_path_close(actual: &str, expected: &str, tolerance: f64) {
        fn coordinates(path: &str) -> Vec<f64> {
            ...
            for segment in SimplifyingPathParser::from(path) {
                match segment.unwrap() {
                    SimplePathSegment::MoveTo { x, y } | SimplePathSegment::LineTo { x, y } => {
                        result.extend([x, y])
                    }
                    SimplePathSegment::CurveTo {
                        ...
                        x,
                        y,
                    } => result.extend([x1, y1, x2, y2, x, y]),
                    SimplePathSegment::Quadratic { x1, y1, x, y } => result.extend([x1, y1, x, y]),
                    SimplePathSegment::ClosePath => {}
                }
            }
            result
        }
        let actual = coordinates(actual);
        let expected = coordinates(expected);
        ...
        let maximum = actual
            ...
            .fold(0.0, f64::max);
        assert!(maximum <= tolerance, "maximum coordinate drift {maximum}");
    }
}
