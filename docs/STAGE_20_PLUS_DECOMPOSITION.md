...
user accepted its ordinary Voronoi authority on 2026-08-25. Stages 20P–20S and
later Stages 21–23 remain Planned and separately gated by their own contracts;
...
canonical-region/render foundations, and concrete ordinary Voronoi regions.
Guide faces, region treatments/sampling, heterogeneous composites, and
multiframe/transition authority remain absent.
...
| Adjacency/topology | Accepted geometry-owned `SiteAdjacency`, nearest/random/tree connection programs, and straight-grid wall-maze face/dual authority | Masks/walks, TSP, general arrangement faces, and composite site-use dependencies remain absent. | 20L/20M are accepted; 20P reuses general face construction and 20R adds derived site-use dependencies. | **Structural:** graph rule/program/seed miss family-derived topology and downstream. |
| Regions/cells/faces | Geometry-owned canonical-region contract, ordinary Voronoi source, and render fill boundary | Guide faces, treatments/sampling, and composites remain absent; canvas must not create cells or close topology. | Completed 20O Voronoi; 20P guide faces and 20Q realization remain planned. | **Structural:** region source/treatment changes miss realization/downstream; final clip stays consumer-only. |
...
| Realization | `PatternOutputLayer`, ordered per-output settings, canonical circle/closed-shape marks, guide paths, connected thickness response, reusable compact stroke outlines, canonical-region fill units, and fixed Full ordinary Voronoi regions | Guide-face sources, region treatments/sampling, and composite output do not exist yet. | Completed 20O regions; 20P guide faces, 20Q treatments, and 20R composites remain planned. | **Realization:** prototype, geometry response, path/region treatment miss realization/downstream while retaining family. |
...
| Canonical geometry/render/export | Canonical circles, closed marks, guide paths, compact filled strokes, ordinary Voronoi canonical regions, renderer scene, and engine preview/raster/SVG consumers | Guide-face sources, treatments/sampling, and composites are missing; renderers must not generate or repair topology and apply only the final canvas clip. | Completed 20O region production; 20P–20Q additions and 20R composites remain planned. | Canonical content follows structural/realization changes; raster target/AA/background are **presentation** consumers. |
