...
parametric site/Voronoi mechanisms remain valid. Stage 20Q adds fill-only
region treatments and sampling. Stage 20R adds ordered heterogeneous outputs,
site-use filters, dependency evaluation, per-output cache identity, and
canonical consumption while retaining headless and final-clip-only boundaries.
Multiframe/transition authority remains absent.
